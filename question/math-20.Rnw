...
und blicken in das puderrote Gesicht \Sexpr{person}. "'Wieso?"', entfährt es
...
Zuerst werden Ihre Fähigkeiten getestet, der Mathematik folgen zu können. Oder berechnen Sie gerade den Einkauf von \Sexpr{person}?\\
...
\item Wenn Sie die \Sexpr{heidel_beere_kauf} Blaubeerschalen gekauft haben, wie viele Aligatorbirnen zu je \Sexpr{aligator_price} EUR können Sie sich dann noch für \Sexpr{schein} EUR leisten? \textbf{(2 Punkte)}
\end{enumerate}

Das Whiteboard beinhaltet folgende Liste mit Informationen zu Produkten aus dem \Sexpr{markt}. Seltsam, was man so alles in einem \Sexpr{markt} über Produkte erfährt.
...
\item Ein Kilo Blaubeeren benötigt \Sexpr{kilo_blaubeeren_water}l Wasser. Eine Blaubeeree wiegt \Sexpr{blaubeeren_g}g.
\end{itemize}
