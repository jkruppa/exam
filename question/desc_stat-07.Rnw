...
count_vec <- str_c(cnt, ": ", str_c(rpois(sample(c(39, 42, 45), 1), 4), collapse = ", ")) |> 
  str_replace("^die ", "Die ")
...
\item Berechnen Sie aus den Daten die \textit{Wahrscheinlichkeit} \Sexpr{lim_word} als die Anzahl \Sexpr{limit} zu beobachten! \textbf{(1 Punkt)}
\item Berechnen Sie aus den Daten die \textit{Chance} \Sexpr{lim_word} als die Anzahl \Sexpr{limit} zu beobachten! \textbf{(1 Punkt)}
