\section{Aufgabe \hfill (12 Punkte)}

\textit{Geben Sie grundsätzlich Formeln und Rechenweg zur Lösung der Teilaufgaben mit an!} \\[1Ex]
 
<<>>=
name <- par_lst$naming |> sample(1)
name_sec <- str_subset(unique(par_lst$naming), name, negate = TRUE) |> sample(3)
avatar_file <- list.files(file.path(exam_path, "avatare"), full.names = TRUE) |> 
  str_subset(str_c(c(name, name_sec), collapse = "|"))
jahr <- par_lst$year
obser <- par_lst$obser |> sample(1) |> magrittr::extract2(1) |> sample(1)
avatar1file <- str_c("\\includegraphics[width = 1.3cm]{", avatar_file[1], "}")
avatar2file <- str_c("\\includegraphics[width = 1.3cm]{", avatar_file[2], "}")
avatar3file <- str_c("\\includegraphics[width = 1.3cm]{", avatar_file[3], "}")
avatar4file <- str_c("\\includegraphics[width = 1.3cm]{", avatar_file[4], "}")
@
 
%% --------------------------------------------------------------------
\ifcollection
\begin{flushright}
\tiny\vspace{-3Ex}
\textbf{\examinhaltstart}
\exammodulemathstat
\vspace{-4Ex}
\end{flushright}
\begin{minipage}[t]{0.5\textwidth}
\Sexpr{avatar1file}\hspace{-4mm}\Sexpr{avatar2file}\hspace{-4mm}\Sexpr{avatar3file}\hspace{-4mm}\Sexpr{avatar4file}
\end{minipage}
\begin{minipage}[t]{0.5\textwidth}
\hfill
\href{https://youtu.be/Fu8kN0Uj13Y}{\includegraphics[width = 2cm]{img/youtube}}
\end{minipage}
{\tiny\textbf{Stichworte:} The Great Horse-Manure Crisis of 1894\footnote{Weitere Informationen: \href{https://fee.org/articles/the-great-horse-manure-crisis-of-1894/}{The Great Horse-Manure Crisis of 1894} und \href{https://www.gregorystrachta.com/18.html}{The Great Horse Manure Crisis of 1894 was never mentioned before 2004}} $\bullet$ Napoleon $\bullet$ Ivenacker Eichen}
\fi
%% --------------------------------------------------------------------



<<>>=

pronom <- lst(sie = c("ihrem", "ihrer", "ihren", "ihr", 
                      "sie", "ihre", "ihr", "die", "Frau"),
              er = c("seinem", "seiner", "seinen", "ihm", 
                     "ihn", "seine", "sein", "der", "Mann"))[[names(name)]]
lerngruppe <- c("Die Pantoffeltieren", "Die Blattläuse", "Die Kühe auf dem Deich",
                "Sinnlos im Studium") |> sample(1)

sentence_name_vec <- sentence_func(name, pronom, attr_lst[[name]])
attr_name_vec <- attr_lst[[name]]
attr_name_sec1_vec <- attr_lst[[name_sec[1]]]
attr_name_sec2_vec <- attr_lst[[name_sec[2]]]
attr_name_sec3_vec <- attr_lst[[name_sec[3]]]

@

\ifcollection
\paragraph{Herodot – der Schimmel aus Ivenack}
\fi

Die Lerngruppe \textit{\Sexpr{lerngruppe}} bestehend aus \Sexpr{name}, \Sexpr{name_sec[1]}, \Sexpr{name_sec[2]} und \Sexpr{name_sec[3]} waren auf Exkursion in Brandenburg und haben dort Folgendes erarbeitet. Während der Besetzung Mecklenburgs durch die Franzosen kamen Napoleon die Geschichten des berühmten Apfelschimmels Herodot, ein Englisches Vollblut, aus Ivenack zu Gehör. Herodot lief zwar niemals Rennen, war aber eines der berühmtesten Pferde dieser Zeit. Napoleon selbst gab den Auftrag, diesen Schimmel durch die Armee nach Frankreich zu bringen. Der Legende nach sollen Arbeiter den Schimmel im hohlen Stamm einer 1000-jährigen Eiche aus Ivenack vor den Franzosen versteckt haben. Doch Herodot verriet sein Versteck durch lautes Wiehern, woraufhin die französische Armee den Schimmel beschlagnahmte und nach Frankreich führte\footnote{Die Quelle der Inspiration  für die Aufgabe war eine Fahrt an die Ostsee und folgender Artikel:
  \href{https://www.wald-mv.de/landingpage/ivenacker-eichen/}{Entdecke das erste Nationale Naturmonument Deutschlands - Ivenacker Eichen und Hutewald}}. Jetzt wollen die vier herausfinden: \textit{"Konnten die Ivenacker den Apfelschimmel Herodot vor dem Zugriff von Napoleon in der 1000-jährigen Eiche verstecken?"} 

<<>>=
today <- as.Date(Sys.Date(), format='%d/%m/%y')
birth <- c(1022, 1112, 1076, 1098, 1201) |> sample(1)
age <- lubridate::year(today) - birth
mm_wachstum <- (c(0.8, 0.9, 1.0, 1.1, 1.2)*2.5) |>  sample(1)
today_r_cm <- 0 + mm_wachstum/10 * age
brust_umfang <- round(2*today_r_cm*pi/100)
year_stolen <- c(1805, 1810, 1815, 1820, 1840) %>% sample(1)
horse_h <- c(180, 185, 190, 195) %>% sample(1)
horse_b <- c(85, 90, 95, 75, 80) %>% sample(1)
horse_l <- c(230, 240, 250, 220) %>% sample(1)
crit_width <- c(10, 15, 20, 30, 25) %>% sample(1)
hero_min <- c(152, 154, 156, 158) |> sample(1)
hero_max <- c(173, 172, 175, 171) |> sample(1)
fitting <- c("mit blutiger Nase", "sehr bequem mit Abstand") %>% sample(1)
## napoleon
nap_h <- c(166, 168, 170, 172) |> sample(1)
nap_w <- c(80, 86, 91, 83) |> sample(1)
nap_bmi <- (nap_w/(nap_h/100)^2) |> round(1)
nap_min <- c(24, 25, 26, 27, 28) |> sample(1)
nap_max <- c(31, 32, 33, 34, 35) |> sample(1)
@

\vspace{1Ex}

Helfen Sie der Lerngruppe \textit{\Sexpr{lerngruppe}} bei der Beantwortung der Forschungsfrage! Es ist bekannt, dass die Eiche im Jahr 2022 einen Umfang von $\Sexpr{brust_umfang}m$ in Brusthöhe hatte.

\begin{enumerate}
\item Skizzieren Sie in einer Abbildung einen linearen Zusammenhang und einen exponentiellen Zusammenhang für das Wachstum der 1000-jährigen Eiche! \textbf{(1 Punkt)}
\item Wie groß war der Durchmesser in $m$ der Eiche im Jahr $\Sexpr{year_stolen}$ als Herodot in der Eiche versteckt werden sollte? \textbf{(1 Punkt)}
\begin{enumerate}
\item Visualisieren Sie in einem Koordinatenkreuz den zeitlichen Zusammenhang $t$ vom Wachstum des Umfangs $U$! \textbf{(1 Punkt)}
\item Erstellen Sie für die Lösung der Frage die lineare Gradengleichung für das Wachstum des Umfangs! \textbf{(1 Punkt)}
\end{enumerate}
\end{enumerate}

Es wurde berichtet, dass sich Herodot in der 1000-jährigen Eiche um die eigene Achse drehen konnte.

\begin{enumerate}
  \setcounter{enumi}{2}
\item Berechnen Sie die Dicke der Eichenwand in $cm$! Verdeutlichen Sie Ihre Berechnungen an einer aussagekräftigen Skizze für Pferd und Eiche! \textbf{(2 Punkte)} 
\item Unter einer Dicke der Eichenwand von $\Sexpr{crit_width}cm$ bricht die Eiche zusammen. Beantworten Sie die Forschungsfrage! Begründen Sie Ihre Antwort! \textbf{(2 Punkte)} 
\end{enumerate}
 
%Herodot hatte eine Schulterhöhe von $\Sexpr{horse_h}$cm, eine Breite von $\Sexpr{horse_b}$cm sowie eine Länge von  %$\Sexpr{horse_l}$cm.

%\begin{enumerate}
%  \setcounter{enumi}{2}
%\item Berechnen Sie das effektive Volumen von Herodot in $m^3$, welches Herodot in der 1000-jährigen Eiche %einnehmen würde! \textbf{(2 Punkte)}
%\end{enumerate}

Napoleon hatte eine Körpergröße von $\Sexpr{nap_h}$cm und einen BMI von $\Sexpr{nap_bmi}$kg/m$^2$ kurz bevor er auf Herodot vor der Schlacht von Waterloo (1815) aufsaß. Ein Reiter in der Schlacht trug inklusive Kleidung, Waffen und persönlichem Gepäck etwa $\Sexpr{nap_min}$ bis $\Sexpr{nap_max}$ Kilogramm am eigenen Körper. Die mittlere Widerristhöhe (das Stockmaß) eines Englischen Vollbluts reicht im ausgewachsenen Zustand meist von $\Sexpr{hero_min}$cm bis  $\Sexpr{hero_max}$cm.

\begin{enumerate}
  \setcounter{enumi}{4}
\item War das Aufsitzen von Napoleon auf Herodot unter Tierschutzgesichtspunkten zulässig? Lösen Sie die Aufgabe graphisch sowie numerisch unter der Nutzung der Funktion\footnote{Zusammenhang zwischen der Widerristhöhe sowie der Gewichtsbelastung: \href{https://www.tierschutz-tvt.de/alle-merkblaetter-und-stellungnahmen/}{Merkblatt Nr. 185 - "Reitergewicht": Beurteilung der Gewichtsbelastung von Pferden (Stand: 01.09.2019)}} Gewichtsbelastung $\sim$ Widerristhöhe! \textbf{(2 Punkte)}
\begin{enumerate}
\item Stellen Sie die Funktion des Zusammenhangs zwischen der Widerristhöhe in cm sowie der maximalen Gewichtsbelastung in kg \uline{graphisch} dar! \textbf{(1 Punkt)}
\item Bestimmen Sie die maximale Gewichtsbelastung von Herodot! \textbf{(1 Punkt)}
\end{enumerate}
\end{enumerate}


<<>>=
## 1)
r_2022 <- brust_umfang*100/(2*pi)
delta_y <- 2022 - year_stolen
growth <- delta_y * mm_wachstum/10
r_napoleon <- r_2022 - growth
u_napoleon <- round(r_napoleon * 2, 2)

## 3)
v_h <- round(horse_h * horse_b * horse_l / 2 / 1e6, 2)

## 4)
r_eiche <- r_napoleon
r_horse <- horse_l / 2
delta_r <- round(r_eiche - r_horse, 2)

## 5)
space <- delta_r - crit_width

herodot_solution_kbl <- tibble("1)" = str_c("$d_{", year_stolen ,"} = ", u_napoleon,"cm$"),
                               "2)" = str_c("r = ", "0 + ", mm_wachstum, "mm * Baumalter"),
                               "3)" = str_c("$V_{eff} = ", v_h,"m^3$"),
                               "4)" = str_c("$s_{Eiche} = ", delta_r,"cm$"),
                               "5)" = str_c("$s_{Platz} = ", space,"cm$"),
                               ) |> 
  kable(booktabs = TRUE, align = "c", escape = FALSE) %>% 
  row_spec(0, bold=TRUE) |> 
  kable_styling(latex_options = "hold_position", font_size = 10)
@

