...
ques_vec <- c(str_c('Um zu Überprüfen, ob die Daten die Annahme ', var_vec, ' genügen, können wir folgende Visualisierung nutzen. Dabei kommt dann auch die entsprechende Regel zur Abschätzung der Annahme ', var_vec, ' zur Anwendung.'),
              ...
              str_c('Ihre Betreuung der Abschlussabreit fragt überraschend in der letzten Besprechung, ob Ihre Messwerte ', var_vec, ' genügen. Sonst könnten Sie ja gar nicht einen t-Test rechnen. Da Ihnen die Zeit wegrennt, entscheiden Sie sich für eien schnelle Visualisierung im Anhang. Welche Visualisierung nutzen Sie und welche Regel kommt zur Abschätzung ', var_vec, ' zur Anwendung?')) |>  sample(1)
@
...
wrong_vec <- c('Einen Barplot. Die Mittelwerte müssen alle auf einer Höhe liegen. Die Fehlerbalken haben hier keine Informationen.',
               'Einen Dotplot. Die Punkte müssen sich wie an einer Perlenschnurr audreihen. Eine Abweichung führt zur Ablehnung der Annahme.',
               'Einen Violinplot. Der Bauch der Violine muss hierbei einen höhren Wert annehmen als der Steg der Violine. Dann kann die Annahme angenommen werden.',
