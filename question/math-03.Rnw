...
\item Ein \Sexpr{names(type_chick)} braucht Platz f{\"u}r sich
  selbst. Berechnen Sie nun die K{\"o}rperfl{\"a}che $a$, die ein Tier
  \Sexpr{names(type_chick)} einnimmt. Welche Annahmen haben Sie f{\"u}r die
  Berechnung der \Sexpr{type_chick}k{\"o}rperfl{\"a}che getroffen? \textbf{(2
    Punkte)}
\end{enumerate}
