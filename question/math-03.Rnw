...
Ebenfalls plant \Sexpr{name} eine neue Biogasanlage für \Sexpr{pronom[3]} Betrieb. Der neue Methantank hat einen Radius $r$ von $\Sexpr{radius}m$. Leider gibt es ein paar bauliche Beschränkungen auf dem Grundstück. Ihr Fundament des
zylindrischen Methantanks kann nur ein Gewicht von maximal $\Sexpr{beton_m}t$ aushalten bevor der Tank wegbricht. \Sexpr{name} rechnen eine Sicherheitstoleranz von $\Sexpr{save_m}\%$ ein beinhaltend das Gewicht des
Methantanks. In flüssiger Form hat Methan bei $-80^\circ\text{C}$ eine Dichte von $\Sexpr{dichte_l}kg/m^3$. Bei $-100^\circ\text{C}$ hat Methan eine Dichte von $\Sexpr{dichte_h}kg/m^3$. \Sexpr{name} betreibt \Sexpr{pronom[6]} Anlage bei $-\Sexpr{grad}^\circ\text{C}$.
