...
## solar
sol_min <- c(0.19, 0.2, 0.21) |> sample(1)
sol_max <- c(0.22, 0.23, 0.24) |> sample(1)
@
...
\item Berechnen Sie die Fläche der schrägen, neuen Solaranlage auf dem \Sexpr{type}! \textbf{(2 Punkte)}
\end{enumerate}

Pro Quadratmeter Dachfläche liefert eine moderne Photovoltaikanlage eine Leistung von etwa $\Sexpr{sol_min}$ bis $\Sexpr{sol_max}$kWp (Kilowattpeak) dabei haben moderne Standardmodule eine Größe von ungefähr 1.7 $\times$ 1.1 Metern.

\begin{enumerate}
  \setcounter{enumi}{2}
\item Mit welcher Leistung kann \Sexpr{name} nach der Montage der Solaranlage auf dem \Sexpr{type} rechnen? Lösen Sie die Aufgabe anhand einer Skizze des Daches! \textbf{(2 Punkte)}
\end{enumerate}
