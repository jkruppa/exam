...
\item Skizzieren Sie die Werte $r_i$, $R_i$ und $A_i$ f{\"u}r zwei nebeneinander
  agierender \Sexpr{type_chick} f{\"u}r ein Verhalten $i$. Nutzen Sie hierf{\"u}r vereinfachte
...
\item Welche Fl{\"a}che $a$ nimmt eine \Sexpr{names(type_chick)} ein? Welche Annahmen haben
  Sie f{\"u}r die Berechnung der \Sexpr{type_chick}fl{\"a}che getroffen? \textbf{(2 Punkte)}
\end{enumerate}
