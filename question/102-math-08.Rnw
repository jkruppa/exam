...
a_grad <- c(54, 53, 52, 51) %>% sample(1)
elle <- c(52.2, 52.4, 52.6)  %>% sample(1)
side <- round(c(18, 19, 20, 21)*100/elle) %>% sample(1)
beruf <- c("Mittelst{\"a}ndler", "Chefredaktuer", "Versicherungsverteter",
           "Finanzbeamter") %>% sample(1)
winkel <- c("steil", "flach") %>% sample(1)
n_slave <- c(2,3,4) %>% sample(1)
krank <- c("R{\"u}cken", "Knie", "Schulter") %>% sample(1)
effizienz <- c(80, 90, 70) %>% sample(1)
schubkarre <- c(90, 100, 110) %>% sample(1)
winkel_rampe <- c(9, 10, 11) %>% sample(1)
@
...
den {\"a}gyptischen Pyramiden \Sexpr{a_grad} Grad auf. Die durchschnittliche
Seitenl{\"a}nge einer Pyramide ist \Sexpr{side} K{\"o}nigsellen. Eine K{\"o}nigselle
misst \Sexpr{elle}cm.
...
\begin{enumerate}
\item Die K{\"o}nigspyramide von Meroe soll eine Seitenl{\"a}nge von \Sexpr{side}
...
chronische \Sexpr{krank}schmerzen entwickelt, als die Sklaven von der
...
\item Berechnen Sie die L{\"a}nge der Rampe mit einem Anstellwinkel von
  $\Sexpr{winkel_rampe}^\circ$! \textbf{(2 Punkte)}
