...
right_2_vec <- c('Da sich der Mittelwert und der Median \\underline{kaum unterscheiden}, liegen vermutlich keine Outlier in den Daten vor. Wir verweden den Datensatz so wie er ist.',
                 'Wenn sich der Mittelwert und der Median \\underline{kaum unterscheiden}, liegen vermutlich keine Outlier in den Daten vor.',
                 'Nach der Betrachtung der Werte \\underline{unterscheiden} sich Mittelwert und Median \\underline{kaum}. Die Daten können so verwendet werden wie sie vorliegen. Es gibt keinen Outlier.') |> sample(1)
