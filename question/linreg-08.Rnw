...
outcome <- c("literary_score", "reading_score", "iq_score") |> sample(1)
risk_factor <- c("body_height", "body_weight", "jump_height") |> sample(1)
...
mixed_formula <- reformulate(response = outcome,
                             termlabels = str_c(risk_factor, " + (1|", names(species), ") + (1|direction)"))
