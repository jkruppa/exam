...
\item Visualisieren Sie anhand von Densityplots einer Normalverteilung folgende {\"A}nderung in den
  Daten.  
  \begin{enumerate}
  \item In einer Abbildung f{\"u}r den Fall $\Sexpr{d_vec[1]}$. Beschriften Sie
    $\Sexpr{d_vec[1]}$ in der Abbildung! \textbf{(2 Punkte)}
  \item In einer Abbildung f{\"u}r den Fall $\Sexpr{s_vec[1]}$. Beschriften Sie
    $\Sexpr{s_vec[1]}$ in der Abbildung! \textbf{(2 Punkte)}
  \end{enumerate}
\end{enumerate}
