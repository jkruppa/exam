...
Leider kennt sich \Sexpr{name} mit der Berechnung eines \Sexpr{type} überhaupt nicht aus. Deshalb braucht \Sexpr{names(name)} bei der Berechnung Ihre Hilfe!
...
\begin{enumerate}
  ...
  \item Schätzen Sie den p-Wert aus der Teststatistik $T_{D}$ und $T_{\alpha = 5\%} = \Sexpr{t_crit}$ ab! Begründen Sie Ihre Antwort! \textbf{(2 Punkte)}
