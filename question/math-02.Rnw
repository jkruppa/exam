...
<<>>=
n_plant <- c(120, 180, 240, 210, 150) %>% sample(1)
n_tisch <- c("zwei", "drei", "vier") |> set_names(c(2, 3, 4)) %>% sample(1)
...
<<>>=
## 2)
n_plant/as.numeric(names(n_tisch))
...
tische_solution_kbl <- tibble("1)" = str_c("$d_{", year_stolen ,"} = ", u_napoleon,"cm$"),
