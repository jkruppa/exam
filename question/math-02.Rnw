...
## anzucht
n_an <- c(4,5,6)%>% sample(1)
m_an <- c(7,8,9)%>% sample(1)
## time
time_s <- c(10, 12, 14, 15)%>% sample(1)
@
...
einen Durchmesser von \Sexpr{topf_d}cm und eine Höhe von \Sexpr{topf_h}cm. Der Kubikmeterpreis für Torf liegt bei \Sexpr{torf_cost} EUR. Helfen Sie der Projektgruppe \textit{\Sexpr{lerngruppe}} bei der Planung des Versuches! 

\begin{enumerate}
\item Sie beginnen mit $\Sexpr{n_an} \times \Sexpr{m_an}$ Anzuchtplatten zur Keimung. Wie viele Anzuchtplatten benötigen Sie? \textbf{(1 Punkt)}
\item Nachdem Sie die Zeit gestoppt haben benötigen Sie \Sexpr{time_s}s für einmal Umsetzen eines Setzlings. Wie lange in Stunden brauchen Sie um alle Pflanzen umzutopfen? \textbf{(1 Punkt)}
\item Skizzieren Sie den Versuchsplan auf \textit{\Sexpr{names(n_tisch)}} Tischen im Gewächshaus! \textit{Skizzieren Sie einen Tisch detailierter und die restlichen Tische schematisch!} \textbf{(2 Punkte)}
