...
kable(data_tbl[1:(nrow(data_tbl)/2),], booktabs = TRUE, align = "c", format = "latex") 
@
\end{center}
\end{minipage}
\begin{minipage}[t]{0.45\textwidth}
...
kable(data_tbl[(nrow(data_tbl)/2+1):(nrow(data_tbl)),], booktabs = TRUE, align = "c", 
      format = "latex") 
@
\end{center}
\end{minipage}
