...
<<>>=
ques_vec <- c('Betrachten wir die Teststatistik aus einem abstrakteren Blickwinkel. Beim statistischen Testen wird das \\textit{signal} mit dem \\textit{noise} aus den Daten $D$ zu einer Teststatistik $T_D$ verrechnet. Welche der Formel berechnet korrekt die Teststatistik $T_D$?',
              'Beim statistischen Testen wird der Effekt \\textit{signal} mit der Varianz \\textit{noise} aus den Daten $D$ zu einer Teststatistik $T_D$ verrechnet. Welche abstrakte Formel können wir nutzen um die beiden Parameter miteinander zu verrechnen?',
              'Das statistische Testen kann auch aus einem abstrakten Winkel betrachtet werden. Hierbei verrechnen wir dann \\textit{signal} mit \\textit{noise} um zu einer Teststatistik $T_D$ zu kommen. Welche abstrakte Formel gibt diesen Zusammenhang wieder?') |>  sample(1)
@
...
wrong_vec <- c('Es gilt $T_D = signal \\cdot noise$. Der Effekt $signal$ wird mit der Varianz $noise$ gewichtet.',
               ...
               'Wir gewichten den Effekt $noise$ mit der Varianz $signal$ und erhalten $T_D = \\sfrac{noise}{signal}$.',
