...
\item Zeichnen Sie folgende statistischen Maßzahlen passend ein! 
  \begin{itemize}
  \item Globale Mittelwert mit $\beta_0$ \textbf{(1 Punkt)}
  \item Mittelwerte der einzelnen Behandlungsstufen \textbf{(1 Punkt)}
  \item Mittelwertsdifferenz der einzelnen Behandlungsstufen mit $\beta_{\Sexpr{x_val[1]}}$, $\beta_{\Sexpr{x_val[2]}}$ und $\beta_{\Sexpr{x_val[3]}}$ \textbf{(1 Punkt)}
  \item Residuen oder Fehler mit $\epsilon$ \textbf{(1 Punkt)}
  \end{itemize}
