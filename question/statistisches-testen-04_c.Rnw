...
var_vec <- c(##"logistische Regression. Sie sch{\"a}tzen ein OR",
             "Gaussian Regression. Sie sch{\"a}tzen einen Mittelwertsunterschied"
  ##"Poisson Regression. Sie sch{\"a}tzen ein RR"
  ) %>%
  sample(1)

s_or_n <- sample(c("Varianz $s_p$", "Fallzahl $n$", "Delta $\\Delta$"), 1)
...
Sie rechnen eine simple \Sexpr{var_vec}. 
