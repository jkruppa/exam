...
ques_vec <- c(str_c('Nachdem Sie in einem Feldexperiment zu Leistungssteigerung von ', plant, ' durchgef{\"u}hrt haben, berechnen Sie den Mittelwert und den Median. Der Mittelwert $\bar{y}$ und der Median $\tilde{y}$ ', type_vec, '. Welche Aussage ist richtig?'),
              '',
              '') |>  sample(1)
@
...
wrong_vec <- c('Da sich der Mittelwert und der Median unterscheiden, liegen vermutlich Outlier in den Daten vor. Wir untersuchen den Datensatz nach auff{\"a}lligen Beobachtungen.',
