...
\begin{enumerate}
  \item Formulieren Sie \Sexpr{question_vec}! \textbf{(1
Punkt)}
...
\item Visualisieren Sie \Sexpr{visual_vec} Beschriften Sie die Abbildung! \textbf{(3 Punkte)} 
\end{enumerate}
