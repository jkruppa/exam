...
groups_vec <- x_val %>% 
  ...
  str_replace_all("\\$", "")
gen_data <- function(...){
  n_vec <- c(4:8) %>% sample(1)
  mean_vec <- c(8, 10, 12, 13) %>% sample(1)
...
ques_vec <-  str_c("'Wir müssen als erstes die Gruppen nach absteigender Effektstärke sortieren!', ruft ", name, " ganz aufgeregt. 'Das habe ich mir gemerkt.', blinzelt ", names(name), " ", name_sec, " an. Mark hebt die Augenbraue. Als ob das jetzt so viel weiter helfen würde.") |> 
  sample(1)
...
Leider kennen sich \Sexpr{name} und \Sexpr{name_sec[1]} mit dem \textit{Compact letter display (CLD)} überhaupt nicht aus. Deshalb brauchen bei der Erstellung Ihre Hilfe!  
