...
x_val <- lst(pflanze = c("durschnittlicher Niederschlag [ml/w]",
...
eff_vec <- sample(c(0.1, 0.5, 1.0, 1.5, 2, 2.5), 1)
...
dd <- genData(n_obs, def) |> 
  as_tibble() |> 
  set_names(c("id", str_replace(x_val, " ", "_"), str_c(y_val, collapse = "_")))
  

preprint <- lm(reformulate(str_replace(x_val, " ", "_"), str_c(y_val, collapse = "_")), data = dd) |> 
  summary()
  
preprint$call <- reformulate(str_replace(x_val, " ", "_"), str_c(y_val, collapse = "_"))

preprint 

fit <- lm(reformulate(str_replace(x_val, " ", "_"), str_c(y_val, collapse = "_")), data = dd)
r2 <- r2(fit)$R2 |> round(2)
...
\item Erklären Sie \textit{kurz} den Begriff \texttt{R-squared}! Was sagt Ihnen der Wert $\Sexpr{r2}$ aus? \textbf{(2 Punkte)}
\end{enumerate}
