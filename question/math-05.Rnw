...
\begin{enumerate}
\item Berechnen Sie die Fallbeschleunigung von vor \Sexpr{dino_years} Millionen Jahren unter der obigen Annahme der gleiche Gewichtskraft $\overrightarrow{F_G}$! \textbf{(1 Punkt)}
