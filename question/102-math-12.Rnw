...
job <- c("Ihrer Surfschule", "Ihrem Krokodilreservat",
         "Ihrer Koalakuschelnschule", "Ihrer Faultierpension") %>% sample(1)
...
\item Die durchschnittliche, australische Lebenserwartung eines Kaninchens
  liegt bei \Sexpr{age} Jahren. Folgende Formel f{\"u}r eine geometrische Serie
  ist gegeben.
  \begin{equation*}
...
\item Wie lange dauert es in Jahren, unter Verwendung der
  ...
  wieder auf \Sexpr{recover}\% der maximal Population angewachsen ist?
  \textbf{(2 Punkte)}
\end{enumerate}
...
Disease Virus (RHDV) kosten \Sexpr{impf_cost}\$ und der durchf{\"u}hrende Arzt
verlangt ca. \Sexpr{dr_cost}\$.
