...
<<>>=
n_sonde <- c("zwei", "drei", "vier") %>% sample(1)
c_voyager <- c(60523, 61198, 62587) %>% sample(1)
build_time <- c(1000, 500, 750, 250) %>% sample(1)
ziv_y <- c(11000, 10000, 12000) %>% sample(1)
...
Milchstra{\ss}e aus. Es gibt $\Sexpr{n_stars}$ Sterne in der Milchstra{\ss}e. Gehen
Sie von einer Lichtgeschwindigkeit von $\Sexpr{c_light}m/s$ aus.
...
\item Berechnen Sie die theoretische Anzahl an Replikationsschritten die
  ben{\"o}tigt werden um mit \textit{einem einzigen Replikationsschritt} die
