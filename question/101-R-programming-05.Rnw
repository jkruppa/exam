...
<<echo = FALSE>>=
trt_name <- c("Behandlungen", "D{\"u}nger", "Sorten", "Futterzus{\"a}tze") %>% sample(1)
level_name <- c("eins bis vier", "1 bis 4", "A bis D") %>% sample(1)
block_name <- c("eins bis drei", "I bis IV") %>% sample(1)
outcome_single <- c("drymatter", "grade", "freshmatter", "P", "Fe", "leaf", "pigs") %>%
  sample(3)
weeks <- c("zwei", "drei", "vier") %>% sample(1)
@ 


Sie wollen einen Datensatz aus Excel in \Rlogo laden. In Ihrem Experiment haben Sie
die \Sexpr{trt_name} \Sexpr{level_name} sowie die Bl{\"o}cke \Sexpr{block_name}
vorliegen. Sie messen die Outcomes \Sexpr{pander::p(outcome_single, wrap = "",
  copula = " und ")} an \Sexpr{weeks} verschiedenen Messterminen.
