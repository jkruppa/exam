...
data_tbl <- map_dfr(1:length(groups_vec), gen_data) %>%  
  set_names(c("treatment", "outcome")) %>%
  mutate(treatment = factor(treatment, labels = groups_vec)) %>% 
  sample_n(size = nrow(.)) %>%  
  set_names(c("Behandlung", "Messwert"))
...
\item Zeichnen Sie in \textit{einer} Abbildung die beiden Boxplots für die zwei Behandlungen von \Sexpr{plant}! Beschriften Sie die Achsen der wissenschaftlichen Fragestellung entsprechend! \textbf{(4 Punkte)} 
