...
block <- c("zwei Blöcken", "drei Blöcken") |>  sample(1)

simple <- c("Complete randomized design (CRD) mit drei Wiederholungen",
            "Latin square design",
            str_c("Randomized complete block design (RCBD) mit ", block))  %>% sample(1)
...
ques_vec <-  c(str_c(name_sec[1], " und ", name_sec[2], " sind bei ", name, " um sich Hilfe für eine Versuchsplanung in \\Rlogo zu holen. Im Hintergrund läuft viel zu laut ", attr_name_vec$music , ". Dabei geht es um den Zusammenhang zwischen der Behandlung ", x1_val, " und dem Messwert ", y_val, " in ", obser, ". Der Versuch soll in ", exp, " ", ort, " durchgeführt werden. Nach ", betreuer, " ist der Messwert ", y_val, " normalverteilt. Die beiden entschieden sich für ein faktorielles Versuchsdesign. Im ersten Schritt überlegt ", name, " ein einfaches experimentelles Design zu probieren. Daher entscheiden sich alle drei für ein \\textit{", simple, "}. Das sollte für den Anfang erstmal reichen. 'Und jetzt, was machen wir jetzt?', ", name_sec[2], " schaut die anderen beiden mit großen Augen an. Die zucken mit der Schulter. Alle mampfen ", attr_name_vec$sweet , "."),
               str_c("Neuer Versuch neues Glück! ", name_sec[1], " und ", name_sec[2], " sind bei ", name, " um sich Hilfe für eine Versuchsplanung in \\Rlogo zu holen. Im Hintergrund läuft viel zu laut ", attr_name_vec$film , ". Daher hat ", attr_name_vec$animal, " schon lange reißaus genommen. In dem neuen Versuch geht es um den Zusammenhang zwischen der Behandlung ", x1_val, " und dem Messwert ", y_val, " in ", obser, ". Der Versuch soll in ", exp, " ", ort, " durchgeführt werden. Immerhin ist der Messswert normalverteilt, was einges einfacher macht. Was es nicht so einfacher macht ist, dass ", name_sec[1], " noch als zusätzliche Herausforderung etwas anderes umtreibt: ", attr_name_sec1_vec$flaw , ". Im ersten Schritt überlegt ", name, " ein einfaches experimentelles Design zu probieren. Daher entscheiden sich alle drei für ein \\textit{", simple, "}. Ob es das jetzt einfacher macht?"),
               str_c(str_replace(attr_name_vec$animal, "^\\w{1}", toupper), " macht mal wieder Randale in ", name, "s Zimmer und rennt davon! ", name_sec[1], " und ", name_sec[2], " sind bei ", name, " in ", ort, " wo der neue, bessere Versuch stattfinden soll. Dabei soll in ", exp, " ", ort, " mit ", obser, " durchgeführt werden. Auf dem Tisch stapeln sich ", attr_name_vec$sweet , " aus Vollkorndinkelmehl. Eine Spezialtät der Komune hier. ", name_sec[1], " hasst Vollkorn wie ", attr_name_sec2_vec$sweet , " geliebt werden. In dem neuen Versuch geht es um den Zusammenhang zwischen der Behandlung ", x1_val, " und dem Messwert ", y_val, ". Immerhin ist der Messwert normalverteilt, was einges einfacher macht. Was es nicht so einfacher macht ist, dass ", name_sec[2], " als zusätzliche Herausforderung noch ", attr_name_sec2_vec$flaw , " mitgebracht hat. Daher entscheiden sich alle drei für ein \\textit{", simple, "}. 'Naja, so viel einfacher ist es dann doch nicht...', merkt ", name_sec[2], " an und sucht ", attr_name_vec$animal, ".")) |> 
  sample(1)
