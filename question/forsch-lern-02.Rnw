...
f2_num <- sample(3, 4, 5) |> sample(1)
var_lst <- c("$\\boldsymbol{f_1}$, $\\boldsymbol{f_2}$ und $\\boldsymbol{y_2}$",
             "$\\boldsymbol{x_1}$ und $\\boldsymbol{y_1}$",
             "$\\boldsymbol{x_1}$ und $\\boldsymbol{y_2}$") |> sample(1) |> unlist()
...
\begin{enumerate}
  ...
  \item Skizzieren Sie die drei übergeordneten Analysebereiche der Statistik passend zur obigen Datentabelle! Beschriften Sie die Abbildungen! \textbf{(4 Punkte)}
