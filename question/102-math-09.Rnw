...
\item Wie viele Wochenenden ben{\"o}tigen Sie, um die Matrix $M$ zu f{\"u}llen, wenn Sie
  jeden Tag \Sexpr{n_matrix} schaffen? \textbf{(1 Punkt)} 
\item Welche Strecke in $km$ legen Sie bei der Bew{\"a}ltigung der
  ...
  \textit{ausdr{\"u}cklich} auch die Entfernungsvektoren mit an! \textbf{(6
    Punkte)}
\item Gehen Sie von einer zus{\"a}tzlichen Suchzeit in Stunden $h$ f{\"u}r die
  Caches an den jeweiligen Orten zur reinen Reisezeit mit Ihrem Cachermobil
  aus. Die Suchzeit f{\"u}r \textit{jeden einzelnen} Cache wird durch die
  Funktion
  \begin{equation*}
