...
gamma_grad <- c(35, 50, 60) %>% sample(1)
...
zu Verf{\"u}gung. Sie starten an dem Ort \Sexpr{start}.
...
\Sexpr{north_south}. Wenn Sie von dem Ort C den Ort
B anpeilen, so liegt der Ort D ungef{\"a}hr $\Sexpr{gamma_grad}^\circ$
...
Orten C und E. Somit liegt der Ort E \Sexpr{north_south} von B. Die Strecke
