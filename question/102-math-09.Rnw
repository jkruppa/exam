...
<<>>=
library(stringi)
l_angel <- c(5, 6, 7) %>% sample(1)
v_bike <- c(15, 17, 19)  %>% sample(1)
h_angel <- l_angel + sample(c(1.6, 1.7, 1.8, 1.9), 1)
...
vec_1_l <- c(3, 4, 5) %>% sample(1)
vec_2_l <- c(4.5, 5.5, 6) %>% sample(1)
vec_fach <- c(1.2, 2.1, 1.3) %>% sample(1)
alpha_grad <- c(20, 30, 40) %>% sample(1)
north_south <- c("n{\"o}rdlich", "s{\"u}dlich")  %>% sample(1)
intercept <- c(0.05, 0.1, 0.2) %>% sample(1)
slope <- c(0.15, 0.25, 0.13) %>% sample(1)
start <- c("A", "B", "C", "D", "E") %>% sample(1)
@
...
wollen diesmal endlich die \Sexpr{direct} \Sexpr{type}challenge durchf{\"u}hren. Die
Reihenfolge der Caches nach \Sexpr{type}wertung gibt daher die von Ihnen
...
Verf{\"u}gung. Sie starten an dem Ort \Sexpr{start}.
...
\Sexpr{north_south}. Vom Ort B betrachtet, bilden die Orte C und D einen
rechten Winkel am Ort B. Der Ort B liegt auf gerader Linie zwischen den
Orten C und E. Somit liegt der Ort E \Sexpr{north_south} von B. Die
...
\item Wie viele Wochenenden ben{\"o}tigen Sie, um die Matrix $M$ mit mindestens
  ...
  pro Tag \Sexpr{n_matrix} schaffen? \textbf{(1 Punkt)} 
\item Welche Strecke in $km$ legen Sie bei der Bew{\"a}ltigung der
  \Sexpr{direct} \Sexpr{type}challenge zur{\"u}ck? Geben Sie
