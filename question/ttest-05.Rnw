...
\item Skizzieren Sie eine Abbildung in der Sie $T_{D}$, $Pr(D|H_0)$, $A=0.95$, sowie $T_{\alpha=5\%} = |\Sexpr{t_crit}|$ einzeichnen! \textbf{(4 Punkte)}
