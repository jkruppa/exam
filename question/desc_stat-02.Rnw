...
\item Zeichnen Sie in \textit{einer} Abbildung die Barplots für die Behandlung von \Sexpr{plant}! Beschriften Sie die Achsen entsprechend!\textbf{(4 Punkte)}
\item Beschriften Sie \textit{einen} Barplot mit den gängigen statistischen Maßzahlen! \textbf{(2 Punkte)}
\item Wenn \Sexpr{name} \textit{keinen Effekt} zwischen den Behandlungen von \Sexpr{plant} erwarten würde, wie sehen dann die Barplots aus? \textit{Antworten Sie mit einer Skizze der Barplots!}
  \textbf{(1 Punkt)}
\end{enumerate}
