...
\item [\textbf{C} \msquare] Die $confounder$ Adjustierung wird durchgef{"u}hrt um bei multiplen Vergleichen den Fehler 1. Art zu kontrollieren. Es wird die Irrtumswahrscheinlichkeit adjustiert, daher das $\alpha$-Niveau.
...
Sie haben folgende unadjustierten p-Werte gegeben: 0.21, 0.03, 0.34 und 0.02. Sie adjustieren die p-Werte nach
Bonferroni. Welche Aussage ist richtig?



\begin{enumerate}
\item [\textbf{A} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.0525, 0.0075, 0.085 und 0.005. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 1.25\% verglichen.
\item [\textbf{B} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.84, 0.12, 1 und 0.08. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{C} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.84, 0.12, 1 und 0.08. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 1.25\% verglichen.
\item [\textbf{D} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.84, 0.12, 1.36 und 0.08. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{E} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.0525, 0.0075, 0.085 und 0.005. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\end{enumerate} 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in library("{}ggpubr"{}): there is no package called 'ggpubr'}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggline(data\_tbl, x = "{}dose"{}, y = "{}len"{}, color = "{}supp"{}, size = 2, : could not find function "{}ggline"{}}}\end{kframe}
...
\item [\textbf{B} \msquare] Ein signifikanter Effekt ist zwischen allen \texttt{dose} Faktorstufen nicht zu erwarten. Eine Interaktion liegt nicht vor. Der $\eta^2$-Wert sollte bei ca. 0 liegen.
...
\item [\textbf{D} \msquare] Ein signifikanter Effekt ist zwischen allen \texttt{dose} Faktorstufen zu erwarten. Durch das Kreuzen der OJ und VC Geraden an der \texttt{dose} Faktorstufe D2 ist keine Interaktion vorhanden.
...
\item [\textbf{C} \msquare] Die ANOVA berechnet die F-Statistik indem die MS der Behandlung durch die MS des Fehlers geteilt werden. Wenn die F-Statistik sich der 0 annähert kann die Nullhypothese nicht abgelehnt werden.
\item [\textbf{D} \msquare] Die ANOVA berechnt die F-Statistik aus den SS Behandlung geteilt durch die SS Fehler.
\item [\textbf{E} \msquare] Die ANOVA berechnet die F-Statistik indem die MS des Fehlers durch die MS der Behandlung geteilt werden. Wenn die F-Statistik sich der 1 annähert kann die Nullhypothese nicht abgelehnt werden.
\end{enumerate} 
...
Berechnen Sie die Korrelation \textit{r} zwischen $x$ mit 18, 4, 7, 8 und 7 sowie $y$ mit 13, 12, 17, 12 und 8.  Nutzen Sie folgende Formel:
...
\item [\textbf{B} \msquare] Es ergibt sich eine Korrelation von 1.11
\item [\textbf{C} \msquare] Es ergibt sich eine Korrelation von -0.11
\item [\textbf{D} \msquare] Es ergibt sich eine Korrelation von 0.11
\item [\textbf{E} \msquare] Es ergibt sich eine Korrelation von 0.01
\end{enumerate} 
...
\textit{r} zwischen $x$ mit 5, 15, 4, 5 und 21 sowie $y$ mit 18, 10, 16, 19 und 15.
\begin{equation*}
  r_{x,y} = \cfrac{s_{x,y}}{s_x \cdot s_y}
\end{equation*}
...
\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich eine Kovarianz von -16.5 sowie eine Korrelation von 0.62
\item [\textbf{B} \msquare] Es ergibt sich eine Kovarianz von -16.5 sowie eine Korrelation von -0.62
\item [\textbf{C} \msquare] Es ergibt sich eine Kovarianz von -4492.12 sowie eine Korrelation von 0.62
\item [\textbf{D} \msquare] Es ergibt sich eine Kovarianz von 16.5 sowie eine Korrelation von 0.38
\item [\textbf{E} \msquare] Es ergibt sich eine Kovarianz von 272.25 sowie eine Korrelation von 0.38
\end{enumerate} 
...
Berechnen Sie den Mittelwert und Standardabweichung von $y$ mit 11, 9, 5, 8 und 8.



\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich 8.2 +/- 1.085
\item [\textbf{B} \msquare] Es ergibt sich 8.2 +/- 4.7
\item [\textbf{C} \msquare] Es ergibt sich 9.2 +/- 1.085
\item [\textbf{D} \msquare] Es ergibt sich 8.2 +/- 2.17
\item [\textbf{E} \msquare] Es ergibt sich 7.2 +/- 2.35
\end{enumerate}
...
Berechnen Sie den Median und das IQR von $x$ mit 24, 9, 9, 17, 20, 7, 22, 16, 2, 19 und 63.



\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich 17 +/- 19
\item [\textbf{B} \msquare] Es ergibt sich 17 [9, 19]
\item [\textbf{C} \msquare] Es ergibt sich 17 +/- 9
\item [\textbf{D} \msquare] Es ergibt sich 19 [9, 19]
\item [\textbf{E} \msquare] Es ergibt sich 19 +/- 9
\end{enumerate} 
...
\item [\textbf{C} \msquare] ... die kleinste Quadrate Methode oder auch least square method genannt.
...
\item [\textbf{B} \msquare] Der Begriff \textit{ordinary mean} beschreibt einen einzelnen Mittelwert eines Levels eines Faktors wohingegen der \textit{marginal mean} die unbedeutenden Mittelwerte beschreibt.
...
\item [\textbf{D} \msquare] Der Begriff \textit{ordinary mean} beschreibt die Mittelwerte eines Faktors wohingegen der \textit{marginal mean} die einzelnen Level betrachtet.
\item [\textbf{E} \msquare] Der Begriff \textit{ordinary mean} beschreibt die einzelnen Mittelwerte der Level eines Faktors wohingegen der \textit{marginal mean} den Mittelwert {"u}ber alle Level des Faktors wiedergibt.
\end{enumerate} 
...
\item [\textbf{C} \msquare] Die Verteilung von Daten. Meistens dem Outcome oder auch $y$ genannt. Die Darstellung ist auch mit einer kleinen Fallzahl möglich.
\item [\textbf{D} \msquare] Die Verteilung von Daten. Meistens dem Outcome oder auch $y$ genannt. Hierbei ist eine große Fallzahl notwendig.
\item [\textbf{E} \msquare] Die Verteilung von Daten. Auch 2 bis 5 Beonachtungen können noch dargestellt werden. Auch wenn es sich streng genommen um keine Darstellugn einer Verteilung handelt.
\end{enumerate}
...
\begin{enumerate}
\item [\textbf{A} \msquare] Die Possion-Regression sch{"a}tzt zwei Verteilungsparameter. Deshalb muss {"u}berpr{"u}ft werden, ob Overdispersion vorliegt. Mit einer gesch{"a}tzen Overdispersion von 3.01 liegt keine Overdispersion vor.
\item [\textbf{B} \msquare] Die Possion-Regression sch{"a}tzt drei Verteilungsparameter. Deshalb muss {"u}berpr{"u}ft werden, ob Overdispersion vorliegt. Mit einer gesch{"a}tzen Overdispersion von 3.01 liegt Overdispersion vor. Die L{"o}sung ist die Nutzung von nur zwei der drei Verteilungsparameter: $\gamma_1$ und $\gamma_3$.
...
\item [\textbf{E} \msquare] Die Possion-Regression sch{"a}tzt nur einen Verteilungsparameter. Deshalb muss {"u}berpr{"u}ft werden, ob Overdispersion vorliegt. Mit einer gesch{"a}tzen Overdispersion von 3.01 liegt keine Overdispersion vor. Overdispersion liegt vor, wenn die gesch{"a}tzte Overdispersion unter 1 liegt.
\end{enumerate}
...
Aussage zu einer gesch{\"a}tzen Overdispersion von 2.48 ist
richtig?
...
\item [\textbf{B} \msquare] Die Effektsch{"a}tzer aus einem Modell, erlauben es nur eine ANOVA zu rechnen. 
...
\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich ein Risk ratio von 0.36, da es sich um ein Anteil handelt.
\item [\textbf{B} \msquare] Es ergibt sich ein Risk ratio von 0.57, da es sich um eine Chancenverh{"a}ltnis handelt.
\item [\textbf{C} \msquare] Es ergibt sich ein Risk ratio von 1.75, da es sich um ein Anteil handelt.
\item [\textbf{D} \msquare] Es ergibt sich ein Risk ratio von 0.57, da es sich um ein Anteil handelt.
\item [\textbf{E} \msquare] Es ergibt sich ein Risk ratio von 0.36, da es sich um eine Chancenverh{"a}ltnis handelt.
\end{enumerate}
...
\begin{enumerate}
\item [\textbf{A} \msquare] Die nicht-parametrische Statistik basiert auf R{"a}ngen. Daher wird jeder Zahl ein Rang zugeteilt. Nur auf den R{"a}ngen wird die Auswertung gerechnet. Daher gibt es auch keinen direkt zu interpretierenden Effektsch{"a}tzer.
...
\item [\textbf{B} \msquare] Wir lesen die Daten ein und mutieren die Daten. Dabei ist wichtig, dass wir nicht das Paket \texttt{tidyverse} nutzen, da dieses Paket veraltet ist. Über die Funktion \texttt{library(tidyverse)} schließen wir das Paket von der Analyse aus.
\item [\textbf{C} \msquare] Wir lesen als erstes die Daten über \texttt{read\_excel()} ein, transformieren die Spalten über \texttt{mutate()} in die richtige Form und können dann abschließend über \text{ggplot()} uns die Abbildungen erstellen lassen.
\item [\textbf{D} \msquare] Wir transformieren die Spalten über \texttt{mutate()} in ein \texttt{tibble} und können dann abschließend über \text{ggplot()} uns die Abbildungen erstellen lassen. Dabei beachten wir das wir keine Faktoren in den Daten haben.
\item [\textbf{E} \msquare] Wir lesen als erstes die Daten über \texttt{read\_excel()} ein, transformieren die Spalten über \texttt{mutate()} in die richtige Form und können dann abschließend über \text{ggplot()} uns die Abbildungen erstellen lassen. Wichtig ist, dass wir keine Faktoren sondern nur numerische Variablen vorliegen haben.
\end{enumerate}
...
\midrule
(Intercept) & 38.46 & 0.27\\
light\_binhigh & 3.37 & 0.41\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item [\textbf{A} \msquare] In einer gaussian Regression kann kein Effekt roh interpretiert werden. Es muss erst eine Confounderadjustierung durchgef{"u}hrt werden.
...
\item [\textbf{C} \msquare] In einer gaussian Regression berechnet man das RR. Daher muss der Sch{"a}tzer des Effektes $\beta_1$ noch quadriert werden. Somit liegt das RR bei 11.33
\item [\textbf{D} \msquare] In einer gaussian Regression muss f{"u}r die Interpretation des Effektes das $\beta_1$ quadriert werden. Somit liegt das OR bei 11.33
\item [\textbf{E} \msquare] In einer gaussian Regression wird die Mittelwertsdifferenz betrachtet. Daher ist der Effekt zwischen den beiden Lichtregimen eine Gewichts{"a}nderung von 3.37
\end{enumerate}
...
\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich eine Fallzahl von 68
...
\begin{enumerate}
\item [\textbf{A} \msquare] Wir machen Aussagen über Populationen!
...
\item [\textbf{D} \msquare] Der gepaarte t-Test wird genutzt, wenn die Differenzen der Beobachtungen verbunden sind und wir dadurch die Unabhängigkeit nicht mehr vorliegen haben.
\item [\textbf{E} \msquare] Der gepaarte t-Test wird gerehnet, wenn die Beobachtungen abhängig voneinander sind. Wir messen jede Beobachtung nur einmal und berechnen dann die Differenz zu dem Mittel der anderen Beobachtungen.
\end{enumerate}   
% -----------------------------------------------------------------------
\clearpage
\begin{graybox}{Rechen- und Textaufgaben}
...
\begin{center}
  ...
  \begin{tabular}{l|c|c|c|c|c}
    ...
    \hline
   \textbf{group}  & 2 & 4.13 &  &  &  \strut\\
    \hline
   \textbf{Residuals}  & 27 & 11.07 &  &  &  \strut\\
  \end{tabular}
\end{center}
...
\begin{center}
  ...
  \begin{tabular}{l|c|c|c|c|c}
     & \textbf{Df} & \textbf{Sum Sq} & \textbf{Mean Sq} & \textbf{F value} & \textbf{Pr(>F)} \strut\\
    \hline
   \textbf{supp}  & 1 & 205.68 &  &  &  \strut\\
    \hline
    \textbf{dose}  & 2 & 2421.05 &  &  &  \strut\\
    \hline
    \textbf{supp:dose}  & 2 & 109.86 &  &  &  \strut\\
    \hline
   \textbf{Residuals}  & 54 & 717.91 &  &  &  \strut\\
  \end{tabular}
\end{center}
...
## trt        2  3.94422 1.972110 4.48935 0.020739 *
## Residuals 27 11.86074 0.439287                   
## ---
...
wurde an 67 Mastschweinen durchgef{\"u}hrt, wobei jedes Tier
...
## Response: len
##           Df   Sum Sq  Mean Sq   F value                 Pr(>F)    
## supp       1  297.844  297.844  22.65782            0.000012311 ***
## dose       2 2796.492 1398.246 106.36853 < 0.000000000000000222 ***
## supp:dose  2   59.407   29.703   2.25962                0.11306    
## Residuals 61  801.863   13.145                                     
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
\end{kframe}
...
Sie rechnen eine einfaktorielle ANOVA mit einem Faktor $f_1$ mit vier Leveln. Nachdem Sie die einfaktorielle ANOVA gerechnet haben, erhalten Sie einen p-Wert von $0.078$ und eine F Statistik mit $F_{calc} = 1.2$. Als Sie sich die Boxplots der Behandlungen anschauen, stellen Sie fest, dass es eigentlich einen Mittelwertsunterschied zwischen dem ersten und zweiten Level geben müsste. Die $IQR$-Bereiche überlappen sich nicht und die Mediane liegen auch weit vom globalen Mittel entfernt.
...
\begin{center}
  ...
  \begin{tabular}{c|c|c|c}
    ...
    \hline
    \textbf{Medikament (nein)} & \phantom{100}  & \phantom{100}  &   87   \strut\\
    \hline
     &  89 &  60 &  149  \strut\\
  \end{tabular}
\end{center}
...
Sie haben folgende Zahlenreihe $y$ vorliegen
$y = \{15, 19, 20, 18, 20, 15, 24\}$. Berechnen Sie folgende
...
\begin{center}
$5, 5, 3, 11, 1, 5, 1, 3, 4, 4, 6, 2, 1, 1, 3$
\end{center}
...
Einflussvariablen ($x$) wurden erhoben: fertilizier, center und N. Als m{\"o}gliche Outcomevariablen stehen Ihnen nun
...
\toprule
stream & longnose & acerage & maxdepth & no3 & do2\\
\midrule
CONOWINGO\_CR & 112 & 27350 & 65 & 6.95 & 8.5\\
BLUE\_LICK\_RUN & 1 & 2217 & 46 & 1.20 & 8.5\\
BEAR\_CR & 54 & 19611 & 96 & 0.99 & 8.3\\
BENNETT\_CR & 2 & 583 & 51 & 2.26 & 9.2\\
\bottomrule
\end{tabular}
\end{table}
...
## -8.8724  -3.9026  -1.4817   1.7128  18.1798  
## 
## Coefficients:
##                Estimate  Std. Error t value Pr(>|t|)   
## (Intercept) 0.802092253 1.309042229  0.6127 0.542887   
## acerage     0.000030822 0.000012283  2.5094 0.015448 * 
## maxdepth    0.005969459 0.004484483  1.3311 0.189305   
## no3         0.215956806 0.063463043  3.4029 0.001337 **
## do2         0.163522222 0.129663315  1.2611 0.213234   
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## (Dispersion parameter for quasipoisson family taken to be 30.331989)
## 
##     Null deviance: 1901.97  on 53  degrees of freedom
## Residual deviance: 1231.62  on 49  degrees of freedom
## AIC: NA
## 
...
##   <chr>           <dbl>     <dbl>   <dbl>
## 1 (Intercept)     0.352    -0.550   0.582
## 2 nematodes       0.129     0.896   0.370
## 3 light           0.438     0.420   0.674
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\midrule
12.44 & 1 & 11.86 & 103.41 & 73.73 & 9\\
10.68 & 0 & 7.48 & 100.87 & 73.27 & 11\\
11.19 & 0 & 10.45 & 104.95 & 76.03 & 9\\
12.18 & 0 & 8.09 & 102.51 & 73.27 & 7\\
\bottomrule
\end{tabular}
\end{table}
...
## Residuals:
##      Min       1Q   Median       3Q      Max 
## -5.51087 -1.51663 -0.05024  1.61380  5.16077 
## 
## Coefficients:
##              Estimate Std. Error t value              Pr(>|t|)    
## (Intercept)  0.045082   7.493507  0.0060             0.9952129    
## nematodes    0.084345   0.075281  1.1204             0.2654626    
## Fe           0.751842   0.071030 10.5849 < 0.00000000000000022 ***
## P           -0.674200   0.168424 -4.0030             0.0001265 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 2.3552 on 92 degrees of freedom
## Multiple R-squared:  0.59507,	Adjusted R-squared:  0.58187 
## F-statistic: 45.067 on 3 and 92 DF,  p-value: < 0.000000000000000222
\end{verbatim}
\end{kframe}
\end{knitrout}
}
...
## Residuals:
##      Min       1Q   Median       3Q      Max 
## -6.22222 -1.47222 -0.68254  1.10714  9.77778 
## 
## Coefficients:
##             Estimate Std. Error t value       Pr(>|t|)    
## (Intercept)  19.2222     1.3640 14.0928 0.000000001158 ***
## trtB         -4.0794     2.0621 -1.9782        0.06793 .  
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 4.0919 on 14 degrees of freedom
## Multiple R-squared:  0.21846,	Adjusted R-squared:  0.16264 
## F-statistic: 3.9134 on 1 and 14 DF,  p-value: 0.067927
\end{verbatim}
\end{kframe}
\end{knitrout}
...
##    Data: data_tbl
## 
## REML criterion at convergence: 800.4
## 
## Scaled residuals: 
##      Min       1Q   Median       3Q      Max 
## -2.12705 -0.67506 -0.07411  0.49472  3.50365 
## 
## Random effects:
##  Groups   Name        Variance Std.Dev.
##  barn     (Intercept)  208.90  14.453  
##  subject  (Intercept) 3941.34  62.780  
##  Residual              715.38  26.747  
## Number of obs: 83, groups:  barn, 7; subject, 6
## 
## Fixed effects:
##             Estimate Std. Error t value
## (Intercept) 203.7805    26.5286  7.6815
## attitudepop -20.7171     5.8768 -3.5253
## 
...
\midrule
A & 16.00 & 15 & 12 & 22 & 13 & 18 & 11.50\\
B & 18.00 & 18 & 11 & 24 & 17 & 22 & 19.50\\
C & 12.11 & 12 & 8 & 20 & 9 & 15 & 16.61\\
D & 12.67 & 13 & 8 & 17 & 13 & 14 & 8.50\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{quote}
  >>Ohne ausreichende Zufuhr von Vitamin C stellen sich nach 73 Tagen die
  ersten Symptome ein; die ersten Toten sind nach 101 Tagen zu beklagen;
  nach 121 Tagen rafft die Skorbut eine ganze Schiffsbesatzung dahin.<<
\end{quote}
...
## t = -3.21159, df = 8, p-value = 0.012395
...
## 
## data:  waterintake by infusion
## t = -3.55371, df = 8, p-value = 0.0074689
## alternative hypothesis: true difference in means between group high and group low is not equal to 0
## 95 percent confidence interval:
##  -11.5423014  -2.4576986
## sample estimates:
...
## t = 0.162938, df = 12, p-value = 0.87328
## alternative hypothesis: true difference in means between group high and group low is not equal to 0
## 95 percent confidence interval:
##  -3.5348761  4.1063046
## sample estimates:
