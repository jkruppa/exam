...
\fancypagestyle{empty}{
  \fancyfoot[L]{\tiny $\square\!\blacksquare\!\blacksquare\!\square\!\blacksquare\!\blacksquare\!\square\!\square\!\blacksquare\!\blacksquare\!\blacksquare\!\square\!\blacksquare\!\square\!\blacksquare\!\square\!\blacksquare\!\blacksquare\!\blacksquare\!\square$}
}
\thispagestyle{empty}
\clearpage
% -----------------------------------------------------------------------
...
\item [\textbf{C} \msquare] Das $\eta^2$ beschreibt den Anteil der Varianz, der von den Behandlungsbedingungen erklärt wird. Daher werden 17\% der Varianz durch die Behandlungsgruppen erklärt.
\item [\textbf{D} \msquare] Es werden 83\% der Varianz durch die Behandlung erklärt. Das $\eta^2$ beschreibt den Anteil der Varianz, der von den unterschiedlichen Behandlungsbedingungen nicht erklärt wird.
...
Gegeben ist $y$ mit 12, 11, 9, 2 und 7. Berechnen Sie den Mittelwert und Standardabweichung.



\begin{enumerate}
\item [\textbf{A} \msquare] Sie erhalten 8.2 +/- 1.98
\item [\textbf{B} \msquare] Es berechnet sich 8.2 +/- 3.96
\item [\textbf{C} \msquare] Es ergibt sich 9.2 +/- 1.98
\item [\textbf{D} \msquare] Es berechnet sich 8.2 +/- 15.7
\item [\textbf{E} \msquare] Es ergibt sich 7.2 +/- 7.85
\end{enumerate} 
...
Berechnen Sie den Median, das $1^{st}$ Quartile sowie das $3^{rd}$ Quartile von $y$ mit 28, 19, 29, 14, 26, 22, 20, 19, 26, 23 und 51.




\begin{enumerate}
\item [\textbf{A} \msquare] Sie erhalten 23 [17; 26]
\item [\textbf{B} \msquare] Sie erhalten 23 +/- 28
\item [\textbf{C} \msquare] Es berechnet sich 25 [20; 29]
\item [\textbf{D} \msquare] Es ergibt sich 23 +/- 19
\item [\textbf{E} \msquare] Sie erhalten 23 [19; 28]
\end{enumerate} 
...
\item [\textbf{D} \msquare] Durch die Abbildung des Boxplot erhalten wir die Informationen über die Mittelwerte und die Varianz.
...
Bevor Sie in Ihrer Abschlussarbeit einen statistischen Test rechnen, wollen Sie einmal betrachten, welcher Verteilung Ihre $n = 190$ geernteten Pflanzen folgen.  Welche Verteilung ist abgebildet?
...
In einer lineren Regression kann es vorkommen, dass der Effekt repräsentiert durch den $\beta$ Koeffizienten nicht so richtig von der Größenordnung zu dem p-Wert passen will. So liefert eine Untersuchung des Einflusses von der $CO_2$-Konzentration in [$\mu g$] im Wasser auf den absoluten Proteingehalt in [$kg$] an Erbsen folgende Effekte und p-Werte: $2e-04$ als p-Wert und einen $\beta_{CO_2}$ Koeffizienten von $2.3\times 10^{-9}$. Welche Aussage ist richtig?
...
\item [\textbf{C} \msquare] Wenn der Effekt $\beta_{CO_2}$ winzig ist, dann kann es an einer falsch gewählten Einheit liegen. Der Anstieg von einer Einheit in $X$ führt ja zu einer Änderung von $\beta_{CO_2}$ in $x$. Wir müssen daher die Einheit von $y$ entsprechend anpassen.
...
Neben der Mittelwertsdifferenz als Effektschätzer bei normalverteilten Endpunkten wird auch häufig der Effektschätzer Odds ratio bei binären Endpunkten verwendet. Welche Aussage über den Effektschätzer Odds ratio ist im folgenden Beispiel zur Behandlung von Klaueninfektionen bei Schweinen richtig? Dabei sind 3 Tiere krank und 7 Tiere sind gesund.



\begin{enumerate}
\item [\textbf{A} \msquare] Es ergibt sich ein Odds ratio von 0.3, da es sich um ein Anteil handelt. Wir berechnen den Anteil der Kranken.
\item [\textbf{B} \msquare] Es ergibt sich ein Odds ratio von 2.33, da es sich um ein Anteil handelt.
\item [\textbf{C} \msquare] Es ergibt sich ein Odds ratio von 0.43, da es sich um eine Chancenverhältnis handelt
  
\item [\textbf{D} \msquare] Es ergibt sich ein Odds ratio von 0.3, da es sich um eine Chancenverhältnis handelt.
\item [\textbf{E} \msquare] Der Anteil der Gesunden wird berechnet. Da es sich um ein Anteil handelt ergibt sich ein Odds ratio von 0.3.
\end{enumerate}
...
Sie haben folgende unadjustierten p-Werte gegeben: 0.42, 0.02, 0.89, 0.34, 0.001 und 0.03. Sie adjustieren die p-Werte nach
Bonferroni. Welche Aussage ist richtig?



\begin{enumerate}
\item [\textbf{A} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 2.52, 0.12, 5.34, 2.04, 0.006 und 0.18. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{B} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.07, 0.0033, 0.1483, 0.0567, 2e-04 und 0.005. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 0.83\% verglichen.
\item [\textbf{C} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.07, 0.0033, 0.1483, 0.0567, 2e-04 und 0.005. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{D} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 1, 0.12, 1, 1, 0.006 und 0.18. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{E} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 1, 0.12, 1, 1, 0.006 und 0.18. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 0.83\% verglichen.
\end{enumerate}
...
Ein Versuch wurde in 17 Parzellen pro Gruppe durchgeführt. Die folgende Abbildung enthält die Daten aus diesem Versuch zur Bewertung der Wirkung des Mikronährstoff Eisen auf den Ertrag in t/ha von Weizen im Vergleich zu einer Kontrolle.  Welche Aussage ist richtig, wenn Sie einen t-Test rechnen?
...
\begin{enumerate}
\item [\textbf{A} \msquare] Es liegt kein signifikanter Unterschied vor. Der Effekt liegt bei 7.
...
Barplots sind bedeutend in der Darstellung von wissenschaftlichen Ergebnissen. Leider hat sich Jonas nicht gemerkt, welche statistischen Maßzahlen für einen Barplot erhoben werden müssen. Das ist in soweit doof, da nach seiner Betreuer erstmal ein Barplot nachgebaut werden soll, bevor es mit seinem Projektbericht losgeht. Dann hat er schonmal den \Rlogo Code vorliegen und nachher geht dann alles schneller. Die Behandlung für Lauch werden verschiedene Genotypen ($AA$, $AB$ und $BB$) sein. Erfasst wird als Outcome ($Y$) \textit{Proteingehalt}. Jonas soll dann \textit{protein} in seiner Exceldatei eintragen.
...
Alex betrachtet die folgenden Daten nach einem Stallexperiment mit Lamas. In dem Experiment wurden die Anzahl an weißen Blutkörperchen gezählt. Nach der Meinung seinem Betreuer muss als erstes geschaut werden, wie diese verteilt sind. Also welcher statistischen Verteilung die Anzahl an weißen Blutkörperchen folgen. Dazu soll Alex ein Histogramm verwenden. Dann hätte man auch einen guten Überblick über das Outcome ($Y$).

\begin{center}
Die Anzahl an weißen Blutkörperchen: 3, 4, 2, 6, 1, 6, 3, 1, 6, 2, 2, 2, 3, 0, 3, 1, 3, 2, 2, 3, 6, 2, 2, 9, 5, 2, 1, 3, 5, 5, 4, 4, 5
\end{center}
...
In einem Gespräch mit ihrem Betreuer wird Yuki gebeten seine Daten aus einem Leistungssteigerungsversuch mit Lamas in einem Histogramm darzustellen. In ihrem Experiment hat er die mittleren dunklen Pigmentstörungen erst fotographiert und dann ausgezählt. Laut ihrem Betreuer soll das Histogramm helfen, die Verteilung der die mittleren dunklen Pigmentstörungen zu bestimmen.

\begin{center}
Die mittleren dunklen Pigmentstörungen: 11.6, 9.7, 8.9, 10.2, 9.5, 10.8, 7.2, 10.3, 10.4, 10.4, 9.5, 9.6, 12.7, 7.4, 9.4, 5.1, 10.1, 7.8, 8.4, 11.7, 6.8, 7.9, 9.1, 8.5, 5.7
\end{center}
...
Alex möchte gerne den Zusammenhang zwischen mittlerer Anzahl an weißen Blutkörperchen [LEU/ml] und Protein/Fettrate [\%/kg] im Kontext von Lamas herausfinden. Hierfür hat Alex ein Stallexperiment im Oldenburger Land durchgeführt. Nach einigen unvorgesehenen Ereignissen hat er es geschafft folgende Datentabelle zu erstellen. Nun stellt sich die Frage für ihn, ob es überhaupt einen Zusammenhang zwischen den gemessenen Variablen gibt. Deshalb möchte Alex als erstes eine explorative Datenanalyse durchführen.
...
Zwei kategoriale Variablen darzustellen ist nicht so einfach. Steffen hatte erst über einen Mittelwert nachgedacht, dann aber die Idee verworfen. In ein Stallexperiment in der Uckermark hatte er sich zum einen die Behandlung Automatische Fütterung [ja/nein] und zum anderen die Messung Protein/Fettrate im Zielbereich [ja/nein] im Kontext von Lamas angeschaut. Jetzt möchte sein Betreuer erstmal die langen Tabellen mit ja/nein in einer explorativen Datenanalyse zusammengefasst bekommen. Sonst geht es bei seiner Abschlussarbeit nicht weiter.
...
\toprule
Automatische Fütterung & Protein/Fettrate im Zielbereich\\
\midrule
nein & ja\\
ja & ja\\
ja & ja\\
...
\item Tragen Sie folgende statistische Fachbegriffe zur statistischen
  ...
  \begin{center}
  \begin{tabular}{cccc}
  H$_0$ falsch & Testentscheidung & 20\% & H$_0$ abgelehnt \\
  \end{tabular}
  \end{center}
...
'Der t-Test testet ein normalverteiltes Outcome ($Y$).', liest Jessica laut. Das hilft jetzt auch nur bedingt weiter. Laut ihrer Betreuerin ist zwar ihr Messwert Gewichtszuwachs in der 1LW normalverteilt, aber wie rechnet sie jetzt einen t-Test? Für ihrer Hausarbeit musste sie ein Stallexperiment mit Lamas im Teuteburgerwald durchführen. Als wäre das nicht schon anstrengend genug gewesen. Jetzt soll sie auch noch testen, ob die Behandlung Elterlinie ($Standard$ und $Xray$) ein signifikantes Ergebnis liefert.
...
Der t-Test. Paula erschaudert. Ein mächtiges Werkzeug in den Händen desjenigen, der einen normalverteilten Messwert ($Y$) hat. Aber erstmal überhaupt den t-Test rechnen können. Wie sah das Experiment von Paula überhaupt aus? Paula hat ein Stallexperiment mit Lamas durchgeführt. Dabei wurde die Behandlung Elterlinie ($Standard$ und $Xray$) an den Lamas getestet. Gemessen hat Paula dann als Messwert Fettgehalt [\%/kg]. Warum der Versuch im Oldenburger Land für ihrer Hausarbeit stattfinden musste, ist ihr bis heute ein Rätsel. Egal. Gibt es jetzt einen Zusammenhang zwischen der Behandlung und Fettgehalt [\%/kg]?
...
Es gibt ja immer die Möglichkeit sich Hilfe zu holen. Das geht natürlich auch immer in einer Hausarbeit. Deshalb arbeiten Jonas und Mark gemeinsam an einer Hausarbeit. Das macht dann auch die Analyse ihres Hauptversuches einfacher. Zwar hat jeder von ihnen noch ein Subthema, aber auch da kann man sich ja helfen. In dem Hauptversuch wurde Folgendes von den beiden gemacht. Jonas und Mark haben sich Lamas angeschaut. Dabei geht um Zusammenhang zwischen Bestandsdichte ($hoch$ und $niedrig$) und Fettgehalt [\%/kg]. Jetzt sollen beide einen gepaarten t-Test rechnen.
...
## t = -2.3, df = 16, p-value = 0.03524
## alternative hypothesis: true  is not equal to [condensed]
## 95 percent confidence interval:
##  -14.0757843  -0.5735663
## sample estimates:
...
Helfen Sie Steffen bei der Interpretation des t-Tests! Sonst geht es auch für Yuki und Jonas nicht weiter.
...
'Mit dem R Paket \texttt{\{emmeans\}} können wir gleich die Gruppenvergleiche rechnen und uns das \textit{compact letter displac}' ausgeben lassen!', verkündet Nilufar sichtlich stolz. 'Nach Meinung der Betreuerin soll es aber nur erstmal ein t-Test sein. Und die Ausgabe ist schon wirr genug.', merkt Tina an. Jonas und Tina sind bei Nilufar um sich in \Rlogo helfen zu lassen. Die beiden waren 2 Monate im Emsland um einen Versuch mit Lamas in einem Stallexperiment durchzuführen. Ziel war es das Outcome ($Y$) Fettgehalt [\%/kg] zu bestimmen.
...
'Wir waren im Wendland um Lamas in einem Stallexperiment zu messen.', Jonas legt das Dokument auf den Tisch und schaut Paula und Tina fragend an. Beide schauen fragend zurück. Die beiden sind zu ihm gekommen, da sie sich nicht mit \Rlogo auskennen und daher Hilfe bei der Interpretation des t-Tests brauchen. Kein Problem. Aber worum es in dem Versuch geht, lässt sich nur aus dem Text in seiner Hand erahnen. Vielleicht hilft da ja die Ausgabe des t-Tests in R weiter.
...
## t = -3.0441, df = 13, p-value = 0.009406
## alternative hypothesis: true  is not equal to [condensed]
## 95 percent confidence interval:
##  -8.890372 -1.509628
## sample estimates:
...
Es gibt ja immer die Möglichkeit sich Hilfe zu holen. Das geht natürlich auch immer in einem Projektbericht. Deshalb arbeiten Jessica und Mark gemeinsam an einem Projektbericht. Das macht dann auch die Analyse ihres Hauptversuches einfacher. Zwar hat jeder von ihnen noch ein Subthema, aber auch da kann man sich ja helfen. In dem Hauptversuch wurde Folgendes von den beiden gemacht. Jessica und Mark haben sich Lamas angeschaut. Dabei geht um Zusammenhang zwischen Bestandsdichte ($hoch$ und $niedrig$) und Fettgehalt [\%/kg]. Jetzt sollen beide einen gepaarten t-Test rechnen. Leider kennen sich beide nicht sehr gut in \Rlogo aus.
...
## t = -5.1604, df = 8, p-value = 0.0008632
## alternative hypothesis: true  is not equal to [condensed]
## 95 percent confidence interval:
##  -11.832141  -4.523414
## sample estimates:
...
Jetzt brauchen Jessica und Mark Ihre Hilfe bei der Berechnung eines gepaarten t-Tests in \Rlogo um ihre Arbeit dann in diesem Semester noch abschließen zu können.
...
'Uff... die einfaktorielle ANOVA. Und wir können jetzt anhand der Visualisuierung sehen, ob da schon was signifikant ist?', Mark hebt die Augenbraue. 'Ja, können wir. Dafür müssen wir aber erstmal in \texttt{\{ggplot\}} uns die Daten anschauen. Oder wir zeichnen es flott mit der Hand. Geht auch.', meint Tina dazu. Mark hatte sich in einen Leistungssteigerungsversuch verschiedene Lamas angeschaut. Dabei ging es herauszufinden, ob es einen Zusammenhang zwischen der Behandlung Lüftungssystem ($keins$, $storm$ und $thunder$) und dem Messwert Gewichtszuwachs in der 1LW gibt.
...
Tina und Paula schauen sich etwas entnervt an. Gemeinsam schreiben die beiden ihre Abschlussarbeit und sollen nun als erstes einmal die Daten mit eine einfaktoriellen ANOVA auswerten damit abgeschätzt werden kann, ob überhaupt signifikante Ergebnisse in den multipen Gruppenvergleichen zu erwarten sind. Die beiden waren im Oldenburger Land um ein Kreuzungsexperiment mit Lamas durchzuführen. Dabei haben Tina und Paula den Messwert Fettgehalt [\%/kg] unter der Behandung Bestandsdichte ($standard$, $eng$, $weit$ und $kontakt$) ermittelt.



\vspace{1ex}

Leider kennen sich Tina und Paula mit Berechnung einer einfaktoriellen ANOVA überhaupt nicht aus. Deshalb brauchen beide bei der Erstellung Ihre Hilfe! 
...
\toprule
    ...
    \midrule
   \textbf{Bestandsdichte}  & 3 & 43.13 &  &  &  \strut\\
   \textbf{error}  & 21 & 158.87 &  &  &  \strut\\
...
\toprule
     & \textbf{Df} & \textbf{Sum Sq} & \textbf{Mean Sq} & \textbf{F value} & \textbf{Pr(>F)} \strut\\
    \midrule
   \textbf{Bestandsdichte}  & 3 & 103.68 &  &  &  \strut\\
   \textbf{Error}  & 31 & 614.21 &  &  &  \strut\\
\bottomrule
  \end{tabular}
\end{center}
...
\midrule
standard & 8 & 6.38 & 3.02\\
eng & 9 & 8.78 & 5.45\\
weit & 8 & 6.38 & 3.54\\
kontakt & 10 & 4.10 & 5.00\\
\bottomrule
\end{tabular}
\endgroup{}
\end{table}
...
Jessica schaut sich fragend in der Bibliothek um. Jessica hatte gehofft, dass jemand hier sein würde, den sie kennt und sich mit \Rlogo auskennt. Wird aber enttäuscht. Jessica war im Oldenburger Land um einen Leistungssteigerungsversuch mit Lamas durchzuführen. Nun möchte ihr Betreuer ihrer Hausarbeit erstmal eine ANOVA sehen und die Ergebnisse präsentiert bekommen. Dabei ging es herauszufinden, ob es einen Zusammenhang zwischen der Behandlung Bestandsdichte ($standard$, $eng$, $weit$ und $kontakt$) und dem Messwert Gewichtszuwachs in der 1LW gibt.
...
## Bestandsdichte  3  91.42  30.472  1.0384 0.3934
## Residuals      24 704.30  29.346
\end{verbatim}
\end{kframe}
\end{knitrout}
...
Jonas steht im Oldenburger Land. Und das ist noch langweiliger als es sich anhört. Wäre es nur so spannend wie bei seinen Kommilitonen, die in Almería waren. Ödnis wohin man nur blickt. Oder eben Lamas. Ja, darum geht es in seinem Projektbericht. Und wäre das nicht noch alles genug, ist sein Experiment auch noch als ein Kreuzungsexperiment komplex geraten. Es wurde der Messwert Schlachtgewicht [kg] mit dem Behandlung Flüssignahrung ($ctrl$, $superIn$ und $flOw$) sowie der Behandlung Bestandsdichte ($standard$ und $kontakt$) untersucht. 'Hmpf', denkt Jonas und ruft 'Und jetzt!?' in die Leere.
...
\begin{center}
  ...
  \toprule
     & \textbf{Df} & \textbf{Sum Sq} & \textbf{Mean Sq} & \textbf{F value} & \textbf{Pr(>F)} \strut\\
    \midrule
   \textbf{Flüssignahrung}  & 3 & 251.15 &  &  &  \strut\\
    \textbf{Bestandsdichte}  & 1 & 5.57 &  &  &  \strut\\
    \textbf{Flüssignahrung:Bestandsdichte}  & 3 & 111.2 &  &  &  \strut\\
   \textbf{Error}  & 18 & 302.72 &  &  &  \strut\\
\bottomrule
  \end{tabular}
\end{center}
...
Es ist schon kurz nach fünf und Yuki wird langsam nervös. Yuki wollte heute Abend noch seine E-Sport Qualifikation schauen. Stattdessen versucht ihr Betreuer die Ausgabe der zweifaktoriellen ANOVA zu visualieren und zu überprüfen, ob es mit der Visualisierung der Daten als Boxplots zusammenpasst. Yuki hatte im Oldenburger Land ein Kreuzungsexperiment mit Lamas durchgeführt. Es gab dabei zwei Behandlungen. Einmal Lüftungssystem ($keins$, $storm$, $tornado$ und $thunder$) sowie als zweite Behandlung Flüssignahrung ($ctrl$ und $flOw$). Gemessen wurde der Messwert ($Y$) Protein/Fettrate [\%/kg]. So kompliziert kann das jetzt doch nicht sein!
...
##                               Df  Sum Sq Mean Sq F value    Pr(>F)
## Lüftungssystem                 2 154.667  77.334  7.1877  0.005076
## Flüssignahrung                 1 310.148 310.148 28.8266 4.207e-05
## Lüftungssystem:Flüssignahrung  2  79.258  39.629  3.6833  0.045613
## Residuals                     18 193.664  10.759
\end{verbatim}
\end{kframe}
\end{knitrout}
...
Alex sitzt schon etwas länger bei seinem Betreuer. So langsam macht Alex sich Gedanken, ob er nicht doch mal anmerken sollte, dass er von CLD noch nie was gehört hat. Aber noch kann gelauscht werden, ein Ende ist erstmal nicht in Sicht! Alex hatte in seiner Abschlussarbeit ein Kreuzungsexperiment durchgeführt. Deshalb sitzt er hier. Also eigentlich nein, deshalb nicht. Alex will fertig werden. Hat er sich doch mit Ernährungszusatz ($ctrl$, $fedX$, $proteinX$ und $getIt$) und Protein/Fettrate [\%/kg] schon eine Menge angeschaut. Alex beugt sich leicht nach vorne. Nein, doch keine Pause. Weiter warten auf eine Lücke im Fluss...
...
Paula sitzt schon etwas länger bei ihre Betreuerin. So langsam macht Paula sich Gedanken, ob sie nicht doch mal anmerken sollte, dass sie von CLD noch nie was gehört hat. Aber noch kann gelauscht werden, ein Ende ist erstmal nicht in Sicht! Paula hatte in ihre Abschlussarbeit einen Leistungssteigerungsversuch durchgeführt. Deshalb sitzt sie hier. Also eigentlich nein, deshalb nicht. Paula will fertig werden. Hat sie sich doch mit Ernährungszusatz ($ctrl$, $fedX$, $proteinX$ und $getIt$) und Gewichtszuwachs in der 1LW schon eine Menge angeschaut. Paula beugt sich leicht nach vorne. Nein, doch keine Pause. Weiter warten auf eine Lücke im Fluss... 'Wir müssen als erstes die Gruppen nach absteigender Effektstärke sortieren!', hört Paula noch aus der Ferne bevor sie einnickt.
...
\item Interpretieren Sie das \textit{Compact letter display (CLD)} für Paula und Steffen! \textbf{(1 Punkt)} 
\end{enumerate}
...
\midrule
\textbf{ctrl} & 1.0000000 & 0.1944316 & 0.5010430 & 0.0504232\\
\textbf{fedX} & 0.1944316 & 1.0000000 & 0.5341380 & 0.3823589\\
\textbf{proteinX} & 0.5010430 & 0.5341380 & 1.0000000 & 0.1670481\\
\textbf{getIt} & 0.0504232 & 0.3823589 & 0.1670481 & 1.0000000\\
\bottomrule
\end{tabular}
\endgroup{}
\end{table}
...
Am Ende war es für Tina in ihrem Projektbericht dann doch kein normalverteiltes Outcome. Das was jetzt etwas doff, da er sich auf eine ANOVA gefreut hatte. Prinzipiell ginge das auch irgendwie, aber nun möchte ihr Betreuer gerne einen $\mathcal{X}^2$-Test auf einer $2x2$-Kreuztabelle berechnet bekommen. Tina hatte sich in ein Stallexperiment $n = 118$ Beobachtungen von Lamas angeschaut. Dabei hat sie als Behandlung \textit{Automatische Fütterung [ja/nein]} bestimmt und zum anderen die Variable \textit{Gewichtszuwachs erreicht [ja/nein]} ermittelt. Jetzt muss Tina mal schauen, wie sie das jetzt rechnet.
...
Am Ende hätte Paula dann doch einen normalverteilten Endpunkt in ihrem Projektbericht nehmen sollen. Vor ihr liegen jetzt die Daten von zwei Variablen als Kategorien oder wie es in \Rlogo so schön heißt, als Faktoren. Aber immerhin, hofft sie das was bei den Daten rausgekommen ist. Gezählt hat Paula einiges mit $n = 143$ Beobachtungen von Lamas. Zum einen hat sie als Behandlung \textit{Außenklimakontakt [ja/nein]} bestimmt und zum anderen die Variable \textit{Gewichtszuwachs erreicht [ja/nein]} ermittelt. Nun möchte ihr Betreuer gerne einen $\mathcal{X}^2$-Test auf einer $2x2$-Kreuztabelle berechnet bekommen.
...
Am Ende war es für Alex in seiner Hausarbeit dann doch kein normalverteiltes Outcome. Das was jetzt etwas doff, da er sich auf eine ANOVA gefreut hatte. Prinzipiell ginge das auch irgendwie, aber nun möchte sein Betreuer gerne einen $\mathcal{X}^2$-Test auf einer $2x2$-Kreuztabelle berechnet bekommen. Alex hatte sich in ein Kreuzungsexperiment $n = 143$ Beobachtungen von Lamas angeschaut. Dabei hat er als Behandlung \textit{Ökologisch [ja/nein]} bestimmt und zum anderen die Variable \textit{Protein/Fettrate im Zielbereich [ja/nein]} ermittelt. Jetzt muss Alex mal schauen, wie er das jetzt rechnet. Nach seinem Experiment erhielt er folgende $2x2$ Kreuztabelle aus seinen erhobenen Daten.
...
'Hä? Hatten wir das als Aufgabe nicht schon mal, das wir aus kontinuierlichen Daten eine Abbildung bauen sollten?', fragt Nilufar. Jonas schaut fragend zurück. 'Kann mich wie immer an nichts erinnern. Können wir trotzdem jetzt erstmal die Daten auswerten? Columbo?', antwortet Jonas leicht angespannt. Die beiden hatten einen Leistungssteigerungsversuch im Teuteburgerwald mit Lamas durchgeführt. Dabei wurden die beiden folgenden Variablen gemessen: durchschnittlicher Tagestemperatur [C/d] und Fettgehalt [\%/kg]. Jetzt haben die beiden eigentlich alles zusammen. \textit{Eigentlich...}

\begin{table}[!h]
\centering
\begin{tabular}{cc}
\toprule
Fettgehalt [\%/kg] & Durchschnittlicher Tagestemperatur [C/d]\\
\midrule
14.8 & 17.4\\
19.0 & 22.6\\
10.2 & 13.2\\
...
Leider kennen sich Nilufar und Jonas mit der linearen Regression für kontinuierliche Daten überhaupt nicht aus. Deshalb brauchen beide bei der Erstellung Ihre Hilfe!
...
\midrule
(Intercept) & 1.94 & 2.96 &  & \\
Durchschnittliche Tagestemperatur & 2.03 & 0.31 &  & \\
\bottomrule
\end{tabular}
\endgroup{}
\end{table}
...
## -2.9533 -0.7948  0.0272  0.8186  3.1758 
## 
## Coefficients:
##                                   Estimate Std. Error t value Pr(>|t|)
## (Intercept)                         0.2649     1.7136   0.155 0.877977
## Durchschnittlicher_Bewegungsscore   0.6772     0.1695   3.995 0.000296
## 
## Residual standard error: 1.495 on 37 degrees of freedom
## Multiple R-squared:  0.3014,	Adjusted R-squared:  0.2825 
## F-statistic: 15.96 on 1 and 37 DF,  p-value: 0.000296
\end{verbatim}
\end{kframe}
\end{knitrout}
...
'Hä? Was ist denn das? Das wird ja immer wilder! Hatten wir das als Aufgabe eine lineare Regression zu rechnen? Wir bauen aus kontinuierlichen Daten eine Abbildung und interpretieren diese dann?', fragt Yuki. Jonas schaut fragend zurück. 'Keine Ahnung... das ist jetzt jedenfalls keine Abbildung von irgendwas sondern eine \Rlogo Ausgabe mit ganz wilden Bezeichnungen...', antwortet Jonas leicht angespannt. Die beiden hatten einen Leistungssteigerungsversuch im Wendland mit Lamas durchgeführt. Dabei wurden die beiden folgenden Variablen gemessen: durchschnittliche Tagestemperatur [C/d] und Schlachtgewicht [kg]. Jetzt haben die beiden eigentlich alles zusammen. \textit{Eigentlich...}, denn mit der \Rlogo Ausgabe haben beide jetzt ein Problem.
...
## t = -9.6982, df = 8, p-value = 1.067e-05
...
Einflussvariablen ($x$) wurden erhoben: rainfall, variety und block. Als m{\"o}gliche Outcomevariablen stehen Ihnen nun
...
Lauch in einem Experiment in einer Klimakammer wollen Sie nun den Versuch eine Nummer gr{\"o}{\ss}er
...
mit insgesamt 3 Leveln dar. Der zweite Faktor mit dem Trockenstress
beinhaltet
4 Level. \\
...
Mark muss seiner Abschlussarbeit mit \Rlogo arbeiten. Deshalb sitzt er jetzt mit Ihnen zusammen und hat einige Fragen zu den \textbf{Grundlagen} in \Rlogo an Sie! Na dann wollen Sie mal helfen. Immerhin will sein Betreuer, dass \Rlogo genutzt wird.\\[1Ex]

Mark fragt: \textit{Was ist der Unterschied zwischen \texttt{library()} und \texttt{packages} und warum brauche ich sowas in \Rlogo?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Wo nutzen wir nochmal die Tilde ($\sim$) in R. Das war irgendwie voll wichtig. Wo wird diese genutzt?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Wie sieht der Pipe-Operator aus und was ist seine Funktion? Gerne mit Beispiel!} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Ich verstehe den Zuweisungs-Operator nicht. Wie sieht der aus und was macht der? Gebe mal ein Beispiel!} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Was ist der Unterschied zwischen einem Objekt, einem Wort und einer Funktion?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Warum brauche ich eigentlich das RStudio und R? Wo ist denn da der Unterschied?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Wie war nochmal der Name der Funktion in dem wir in R Daten intern abspeichern? Was waren da nochmal die Vorteile?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]

Mark fragt: \textit{Hä? Warum ändert sich nichts an meinen Daten? In R sehe ich doch die Änderungen aber irgendwie speicher R meine Änderungen meines Datensatzes ab. Was ist da los?} \textbf{(1 Punkt)}\\[1ex]
Sie antworten:\\[2Ex]
...
Leider kennt sich Yuki mit dem Lesen einer wissenschaftlichen Veröffentlichung mit Fokus auf die Statistik überhaupt nicht aus. Deshalb braucht sie bei der Erstellung Ihre Hilfe!
...
\begin{enumerate}
\item Wenn 3 Blaubeerschalen 5.37 Euro kosten,  wie viel kosten 5 Schalen? \textbf{(1 Punkt)}
\item Wenn Sie die 5 Blaubeerschalen gekauft haben, wie viele Aligatorbirnen zu je 2.89 EUR können Sie sich dann noch für 50 EUR leisten? \textbf{(1 Punkt)}
\end{enumerate}
...
\item Ein Kilo Avocado benötigt 1050l Wasser. Eine Avocado wiegt 130 - 420g.
...
Chile exportiert im großem Ausmaß Blaubeeren und Avocados. In dem Exportjahr 2024 blieben die Erträge von Blaubeeren mit \ensuremath{9\times 10^{4}}t in dem prognostizierten Rahmen. Die Menge reduzierte sich um 5.1\%. Die Exporte für Avocados stiegen in dem gleichen Zeitraum um 22.1\% auf \ensuremath{2\times 10^{5}}t.
...
\midrule
preening & 29cm; 20cm; 0.5\% & 32cm; 23cm; 0.5\% & 32cm; 33cm; 0.5\%\\
drinking/eating & 35cm; 25cm; 4.1\% & 38cm; 27cm; 3.6\% & 40cm; 33cm; 3.6\%\\
wingflapping & 34cm; 23cm; 1.2\% & 39cm; 26cm; 1.2\% & 36cm; 21cm; 1\%\\
dustbathing & 31cm; 20cm; 12.8\% & 36cm; 27cm; 14.2\% & 31cm; 30cm; 16.4\%\\
\bottomrule
\end{tabular}
\end{table}
...
Radius von $3500$m kollabiert, wird die Sonne $20$\%
...
\begin{center}
  \begin{tabular}{ ccc }
    ...
    \midrule
    A & GCZOWIJ & 4.5 | 4.0 | Klein \\
    B & GCRNHTI & 3.0 | 3.5 | Normal \\ 
    C & GCKML4T & 1.5 | 1.0 | Klein \\ 
    D & GCLRL8L & 2.0 | 2.0 | Normal \\ 
    E & GCQCNSM & 1.0 | 5.0 | Normal \\     
 \bottomrule
\end{tabular}
\end{center}
...
man nicht alles im Jahr 524 n. Chr. f{\"u}r den neuen Lehnsherren
...
\begin{equation*}
  f(t) = \ensuremath{9\times 10^{9}} - \ensuremath{1.2\times 10^{9}} \cdot 1.7^{-0.3 \cdot t + 2.7}
\end{equation*}
