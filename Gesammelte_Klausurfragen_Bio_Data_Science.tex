...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: <text>:1:138: unexpected '\}'\\\#\# 1: ques\_vec <- c('Betrachten wir die Teststatistik aus einem abstrakteren Blickwinkel. Beim statistischen Testen wird das \textbackslash{}textit\{"{}`signal"{}'\}\\\#\# \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textasciicircum{}}}\end{kframe}
\end{knitrout}
...
\item [\textbf{D} \msquare] Es gilt $T_D = \cfrac{signal}{noise^2}$. Der Effekt $signal$ wird mit der quadratischen Varianz $noise$ gewichtet.
...
Welche Aussage über die Entscheidung anhand der berechneten Teststatistik gegen die Nullhypothese ist richtig?
...
\begin{enumerate}
\item [\textbf{A} \msquare] Anhand der berechneten Teststatistik lässt sich wie folgt eine Entscheidung treffen. Ist der Wert größer als der kritische Wert $T_{\alpha = 5\%}$ dann kann die Nullhypothese abgelehnt werden.
...
\item [\textbf{C} \msquare] Das Signifikanzniveau $\alpha$ ist gleich $5\%$. $Pr(D|H_0)$ muss kleiner sein als das Signifikanzniveau.
\item [\textbf{D} \msquare] Das Signifikanzniveauintervall $\alpha$ ist gleich $5\%$ und damit muss der berrechnete Wert unter dem Signifikanzniveauintervall $\alpha$ liegen, dann kann die Nullhypothese nicht abgelehnt werden.
\item [\textbf{E} \msquare] Anhand der berechneten Teststatistik lässt sich wie folgt eine Entscheidung treffen. Ist die Null mit enthalten, dann kann die Nullhypothese abgelehnt werden.
\end{enumerate}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: '\textbackslash{}e' is an unrecognized escape in character string (<text>:1:258)}}\end{kframe}
\end{knitrout}
