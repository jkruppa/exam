...
\begin{alltt}
\hlstd{pacman}\hlopt{::}\hlkwd{p_load}\hlstd{(tidyverse, simstudy, knitr, xtable, kableExtra, readxl, plyr,}
               \hlstd{gridExtra, broom, effectsize, gtsummary, glmmTMB, performance,}
               \hlstd{janitor, pander, lme4, corpcor, fs)}
\hlcom{##}
\hlstd{opts_chunk}\hlopt{$}\hlkwd{set}\hlstd{(}\hlkwc{fig.width}\hlstd{=}\hlnum{7}\hlstd{,} \hlkwc{fig.height}\hlstd{=}\hlnum{7}\hlstd{,} \hlkwc{fig.path}\hlstd{=}\hlstr{'img/'}\hlstd{,}
               \hlkwc{echo}\hlstd{=}\hlnum{FALSE}\hlstd{,} \hlkwc{warning}\hlstd{=}\hlnum{FALSE}\hlstd{,} \hlkwc{message}\hlstd{=}\hlnum{FALSE}\hlstd{,}
               \hlkwc{cache.path}\hlstd{=}\hlstr{'cache/'}\hlstd{,} \hlkwc{fig.pos}\hlstd{=}\hlstr{'H'}\hlstd{)}

\hlstd{exam_path} \hlkwb{<-} \hlkwd{file.path}\hlstd{(}\hlkwd{path_home}\hlstd{(),} \hlstr{"Documents/GitHub/exam"}\hlstd{)}
\hlstd{init_path} \hlkwb{<-} \hlkwd{file.path}\hlstd{(exam_path,} \hlstr{"init"}\hlstd{)}
\hlstd{question_path} \hlkwb{<-} \hlkwd{file.path}\hlstd{(exam_path,} \hlstr{"question"}\hlstd{)}
\hlstd{img_path} \hlkwb{<-} \hlkwd{file.path}\hlstd{(question_path,} \hlstr{"img"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{graybox}{Checkbox f{\"u}r die Version vom \today}
  ...
  Choice Fragen sowie \textbf{53} Rechen- und
  Textaufgaben.
\end{graybox}
...
\begin{graybox}{Erlaubte Hilfsmittel f{\"u}r die Klausur}
  \vspace{1Ex}
  \begin{itemize}
  \item Normaler, nicht-programmierbarer Taschenrechner (kein Handy!)
  \item Eine DIN A4-Seite \textbf{einseitig} als selbstgeschriebene,
    handschriftliche
    Formelsammlung. % ausschlie{\ss}lich mit Formeln und ohne erkl{\"a}rende S{\"a}tze;
    % keine Beispielrechnungen
  \end{itemize}
\end{graybox}
\vfill
...
Sie haben folgende unadjustierten p-Werte gegeben: 0.001, 0.42, 0.34, 0.21 und 0.02. Sie adjustieren die p-Werte nach
Bonferroni. Welche Aussage ist richtig?



\begin{enumerate}
\item [\textbf{A} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.005, 1, 1, 1 und 0.1. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{B} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.005, 1, 1, 1 und 0.1. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 1\% verglichen.
\item [\textbf{C} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.0002, 0.084, 0.068, 0.042 und 0.004. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{D} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.005, 2.1, 1.7, 1.05 und 0.1. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 5\% verglichen.
\item [\textbf{E} \msquare] Nach der Bonferroni-Adjustierung ergeben sich die adjustierten p-Werte von 0.0002, 0.084, 0.068, 0.042 und 0.004. Die adjustierten p-Werte werden zu einem $\alpha$-Niveau von 1\% verglichen.
\end{enumerate} 
...
\item Beschriften Sie \textit{einen} der beiden Boxplots mit den g{\"a}ngigen
  statistischen Ma{\ss}zahlen. Beschriften Sie auch die Achsen! \textbf{(2 Punkte)}
\item Wenn Sie \textit{keinen Effekt} zwischen de D{\"u}ngestufen erwarten
...
\begin{center}
  \includegraphics[width=10cm]{C:/Users/jokruppa/Documents/GitHub/exam/question/img/boxplot-03-c.png}
\end{center}
...
Sie haben folgende Zahlenreihe $x$ vorliegen
$x = \{19, 18, 18, 17, 14, 16\}$.

\begin{enumerate}
\item Visualisieren Sie den Mittelwert von $x$ in der untenstehenden
  Abbildung! \textbf{(4 Punkte)}
...
$y = \{15, 12, 17, 16, 18, 17, 16, 14, 15, 13, 15\}$. Berechnen Sie folgende
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: <text>:5:0: unexpected end of input\\\#\# 3: \ \ \ \ \ \ \ \ \ \ \ \ \ pattern = "{}diag-doppelbaum"{}, full.names = TRUE)\\\#\# 4: \\\#\# \ \ \textasciicircum{}}}\end{kframe}
\end{knitrout}
...
\begin{center}
  \includegraphics[width=17cm]{C:/Users/jokruppa/Documents/GitHub/exam/question/img/boxplot-03-c.png}
\end{center}
...
Einflussvariablen ($x$) wurden erhoben: dry, weight und center. Als m{\"o}gliche Outcomevariablen stehen Ihnen nun
...
In einem Stallexperiment mit $n = 127$ Ferkeln wurde der
...
\toprule
stream & longnose & no3 & so4 & temp & acerage\\
\midrule
BUFFALO\_RUN & 16 & 0.61 & 18.87 & 16.8 & 12620\\
CABIN\_JOHN\_CR & 21 & 1.57 & 16.09 & 15.0 & 8612\\
PRETTYBOY\_BR & 19 & 6.81 & 9.20 & 19.2 & 904\\
MAINSTEM\_PATUXENT\_R & 239 & 3.31 & 5.95 & 18.1 & 8636\\
\bottomrule
\end{tabular}
\end{table}
...
## -8.54568  -4.62257  -1.43530   0.53138  20.40056  
## 
## Coefficients:
##                 Estimate   Std. Error t value Pr(>|t|)  
## (Intercept)  1.910692842  0.986516355  1.9368  0.05855 .
## no3          0.156262032  0.072495502  2.1555  0.03606 *
## so4         -0.035100310  0.030081539 -1.1668  0.24892  
## temp         0.068006105  0.052955606  1.2842  0.20511  
## acerage      0.000039509  0.000017090  2.3118  0.02504 *
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## (Dispersion parameter for quasipoisson family taken to be 42.086008)
## 
##     Null deviance: 2179.58  on 53  degrees of freedom
## Residual deviance: 1562.80  on 49  degrees of freedom
## AIC: NA
## 
...
## 2 nematodes       0.129     -1.45   0.146
## 3 light           0.433      1.14   0.256
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\midrule
10.48 & 0 & 8.92 & 101.63 & 67.50 & 7\\
9.95 & 0 & 10.80 & 100.65 & 67.96 & 17\\
10.03 & 0 & 10.81 & 97.92 & 69.48 & 6\\
7.20 & 1 & 10.84 & 99.35 & 71.70 & 14\\
\bottomrule
\end{tabular}
\end{table}
...
## Residuals:
##     Min      1Q  Median      3Q     Max 
## -4.3784 -1.6563  0.1649  1.6164  4.4909 
## 
## Coefficients:
##              Estimate Std. Error t value             Pr(>|t|)    
## (Intercept) -6.113297   7.144301 -0.8557              0.39422    
## P           -0.429221   0.171343 -2.5050              0.01386 *  
## light        0.893835   0.480603  1.8598              0.06585 .  
## Fe           0.800358   0.070973 11.2769 < 0.0000000000000002 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 2.2434 on 100 degrees of freedom
## Multiple R-squared:  0.56321,	Adjusted R-squared:  0.5501 
## F-statistic:  42.98 on 3 and 100 DF,  p-value: < 0.000000000000000222
\end{verbatim}
\end{kframe}
\end{knitrout}
}
...
## Residuals:
##    Min     1Q Median     3Q    Max 
##  -1.60  -1.35  -0.60   0.15   4.40 
## 
## Coefficients:
##             Estimate Std. Error t value     Pr(>|t|)    
## (Intercept) 14.60000    0.92736 15.7436 0.0000002647 ***
## trtB        13.00000    1.31149  9.9124 0.0000090641 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 2.0736 on 8 degrees of freedom
## Multiple R-squared:  0.92471,	Adjusted R-squared:  0.9153 
## F-statistic: 98.256 on 1 and 8 DF,  p-value: 0.0000090641
\end{verbatim}
\end{kframe}
\end{knitrout}
...
## Scaled residuals: 
##      Min       1Q   Median       3Q      Max 
## -2.36970 -0.55541 -0.03612  0.62165  3.52857 
## 
## Random effects:
##  Groups   Name        Variance Std.Dev.
##  barn     (Intercept)  238.62  15.447  
##  subject  (Intercept) 4134.34  64.299  
##  Residual              635.13  25.202  
## Number of obs: 83, groups:  barn, 7; subject, 6
## 
## Fixed effects:
##             Estimate Std. Error t value
## (Intercept) 202.9614    27.1711  7.4698
## attitudepop -19.7464     5.5375 -3.5660
## 
...
\midrule
A & 17.57 & 18 & 17 & 18 & 17.0 & 18.0 & 0.29\\
B & 19.29 & 20 & 16 & 22 & 18.5 & 20.0 & 3.57\\
C & 11.00 & 11 & 7 & 15 & 9.5 & 12.5 & 7.00\\
D & 7.57 & 8 & 0 & 13 & 6.5 & 9.5 & 16.95\\
\bottomrule
\end{tabular}
\end{table}
...
\item Skizieren Sie in die
  ...
  \begin{itemize}
  \item[(a)] Ein Konfidenzintervall mit h{"o}herer Fallzahl $n$ in der Stichprobe als der Rest der Konfidenzintervalle 	
...
## t = 2.22566, df = 16, p-value = 0.040763
## alternative hypothesis: true difference in means between group high and group low is not equal to 0
## 95 percent confidence interval:
##  0.10559091 4.33885353
## sample estimates:
...
## 
## data:  drymatter by P
## t = -3.36247, df = 12, p-value = 0.0056483
## alternative hypothesis: true difference in means between group high and group low is not equal to 0
## 95 percent confidence interval:
##  -8.0201660 -1.7131673
## sample estimates:
...
\item Folgende statistische Fachbegriffe bzw. Methoden sind gegeben:
  \begin{itemize}
  \item[] Spearman Korrelation, Wilcoxon-Test, Simple Gaussian linear Regression, t-Test, Kruskal-Wallis-Test, Mann-Whitney-U-Test, ANOVA, Pearson Korrelation und Paired t-Test.
  \end{itemize}
