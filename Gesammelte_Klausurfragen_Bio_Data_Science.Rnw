...
## global parameters
naming_global <- c("Jessica", "Paula", "Tina", "Nilufar",
                   "Mark", "Jonas", "Alex", "Steffen") |> 
  set_names(c("sie", "sie", "sie", "sie", 
              "er", "er", "er", "er")) 
...
par_lst <- lst(year = "2024",
               naming = naming_global |> sample(1),
               naming_sec = str_subset(naming_global, naming, negate = TRUE) |> sample(1),
               obser = c(plant, animal) |> sample(1))
