...
\begin{graybox}{Checkbox für die Version vom \today}
  ...
  Textaufgaben. Somit \textbf{\Sexpr{length(text_questions_vec) + length(mc_questions_vec)}} Aufgaben zur Statistik.\\[1Ex]
  Im Weiteren finden sich \textbf{\Sexpr{length(math_questions_vec)}} Aufgaben zur Mathematik.
\end{graybox}
